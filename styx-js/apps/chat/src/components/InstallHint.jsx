...
export default function InstallHint({ dismissed: vaultDismissed, onDismiss }) {
  ...
  const effectiveDismissed = typeof vaultDismissed === 'boolean' ? vaultDismissed : dismissed;
  ...
  const dismiss = async () => {
    setDismissed(true);
    let result = null;
    try { result = await onDismiss?.(); } catch { /* legacy remains authoritative */ }
