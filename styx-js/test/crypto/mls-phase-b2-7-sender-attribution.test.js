...
import {
  ...
  messageInstanceKey,
  messageStateKey,
} from '../../spikes/marmot-phase-b2-7/b2-7-canonical.mjs';
import { createB27JournalForDb }
  from '../../spikes/marmot-phase-b2-7/b2-7-journal.mjs';
import { createB27Coordinator }
  from '../../spikes/marmot-phase-b2-7/b2-7-coordinator.mjs';
import { EDGE_FIELDS, buildEdge, buildInbound, buildInput, buildMessageState,
  buildRetainedState, parseEdge, parseInput }
  from '../../spikes/marmot-phase-b2-7/b2-7-record.mjs';
...
async function loadWasm() {
  if (!wasmPromise) {
    wasmPromise = (async () => {
      ...
      moduleUrl.searchParams.set('b2-7-convergence-test', '1');
...
async function setupLeafReuseGroup(wasm) {
  const fixture = await setupGroup(wasm);
  const { peers, groupId } = fixture;
  const removedCharlie = peers.charlie;
  const removedMember = rosterMember(peers.alice.group, removedCharlie.publicKey);
  let pending;
  let projection;
  let tree;
  let parsedTree;
  try {
    pending = peers.alice.group.prepare_remove(
      peers.alice.provider, peers.alice.identity, removedMember.senderLeafIndex);
    const removeCommit = copyBytes(pending.commit());
    mergeInbound(peers.bob.group, peers.bob.provider, removeCommit);
    mergeInbound(peers.diana.group, peers.diana.provider, removeCommit);
    projection = pending.projection();
    peers.alice.group.confirm_pending(
      peers.alice.provider, pending, projection.verified_leaf_digest());
    free(projection); free(pending); projection = null; pending = null;

    const provider = new wasm.Provider();
    const readdedCharlie = {
      ...createPeer(wasm, provider, 3, 1_786_435_300),
      provider,
      group: null,
    };
    pending = peers.alice.group.prepare_add(
      peers.alice.provider, peers.alice.identity, readdedCharlie.keyPackage);
    const addCommit = copyBytes(pending.commit());
    const welcomeBytes = copyBytes(pending.welcome());
    mergeInbound(peers.bob.group, peers.bob.provider, addCommit);
    mergeInbound(peers.diana.group, peers.diana.provider, addCommit);
    projection = pending.projection();
    peers.alice.group.confirm_pending(
      peers.alice.provider, pending, projection.verified_leaf_digest());
    tree = peers.alice.group.export_ratchet_tree();
    parsedTree = wasm.PhaseB2RatchetTree.from_bytes(tree.to_bytes());
    readdedCharlie.group = wasm.PhaseB2Group.join(
      readdedCharlie.provider, welcomeBytes, parsedTree);
    peers.removedCharlie = removedCharlie;
    peers.charlie = readdedCharlie;
    for (const peer of [peers.alice, peers.bob, peers.charlie, peers.diana]) {
      peer.snapshotBytes = copyBytes(peer.provider.serialize_state());
    }
    const readdedMember = rosterMember(peers.bob.group, readdedCharlie.publicKey);
    return Object.freeze({ ...fixture, removedMember, readdedMember });
  } catch (error) {
    fixture.cleanup();
    throw error;
  } finally {
...
function rosterMember(group, senderPublicKey) {
  const senderIdentityHex = bytesToHex(senderPublicKey);
  const matches = [];
  for (let index = 0; index < group.member_count(); index += 1) {
    if (bytesToHex(group.member_identity(index)) === senderIdentityHex) {
      matches.push({
        senderLeafIndex: group.member_leaf_index(index),
        senderIdentityHex,
        senderSignatureKeyHex: bytesToHex(group.member_signature_key(index)),
        senderIdentityProofHex: bytesToHex(group.member_identity_proof(index)),
      });
    }
  }
  if (matches.length !== 1) throw new Error('test sender is absent or ambiguous');
  return Object.freeze(matches[0]);
}

function expectDurableSender(delivery, expected) {
  expect(delivery).toEqual(expect.objectContaining(expected));
  expect(delivery.epochDec).toMatch(/^\d+$/);
  expect(delivery.groupContextDigestHex).toMatch(/^[0-9a-f]{64}$/);
  expect(delivery.verifiedLeafDigestHex).toMatch(/^[0-9a-f]{64}$/);
}

function expectNoSenderAuthority(result) {
  for (const field of [
    'epochDec',
    ...
    'senderIdentityProofHex',
    'plaintextBytes',
  ]) expect(result).not.toHaveProperty(field);
}

function wasmWithReceiveOverride(wasm, override) {
  return Object.freeze({
    ...wasm,
    PhaseB2Group: Object.freeze({
      load(provider, groupId) {
        const group = wasm.PhaseB2Group.load(provider, groupId);
        if (group === undefined) return undefined;
        return new Proxy(group, {
          get(target, property) {
            if (property === 'receive_application_message') {
              return (receiveProvider, ciphertext) =>
                override(target.receive_application_message(receiveProvider, ciphertext));
            }
...
function overrideReceivedField(received, field, replacement) {
  return new Proxy(received, {
    get(target, property) {
      if (property === field) return () => replacement(target);
      const value = target[property];
      return typeof value === 'function' ? value.bind(target) : value;
    },
  });
...
function clientFor(wasm, peer, groupId, tag, hooks = {}) {
  ...
  Object.defineProperty(db, 'name', { value: `${B27_DB_PREFIX}${tag}` });
  const journal = createB27JournalForDb(db);
  const coordinator = createB27Coordinator({ journal, wasm, ...coordinatorHooks });
...
function restartClient(wasm, client, hooks = {}) {
  const journal = createB27JournalForDb(client.db);
  return { ...client, journal,
    coordinator: createB27Coordinator({ journal, wasm, ...hooks }) };
}
...
function rebuildEdge(record, changes) {
  const fields = {};
  for (const field of EDGE_FIELDS) {
    if (!['format', 'version', 'edgeDigestHex'].includes(field)) fields[field] = record[field];
  }
  return buildEdge({ ...fields, ...changes });
}
...
describe('Phase B2.7 retained-history convergence', () => {
  test('Stage 2 runtime source uses only the sender-preserving receive API', () => {
    ...
    const runtimeFiles = [
      ...
      'b2-7-record.mjs',
      'b2-7-journal.mjs',
      'b2-7-engine-adapter.mjs',
      'b2-7-coordinator.mjs',
    ];
    const sources = runtimeFiles.map((name) => readFileSync(
      join(HERE, `../../spikes/marmot-phase-b2-7/${name}`), 'utf8'));
    expect(sources.join('\n')).not.toContain('process_application_message');
    expect(sources[3]).toContain('receive_application_message');
  });
  ...
  test('message-state codec rejects the unreachable RELEASED disposition', () => {
    ...
    expect(() => buildMessageState({
      ...
      state: 'RELEASED',
    })).toThrow(expect.objectContaining({ code: B27_ERROR.INVALID }));
  });

  test('accepted inbound codec binds exact sender authority and every non-accepted state rejects it',
    () => {
      const ciphertextBytes = Uint8Array.of(1, 2, 3);
      const plaintextBytes = UTF8.encode('bound');
      const accepted = {
        groupIdHex: '11'.repeat(32),
        instanceKeyHex: '22'.repeat(32),
        ...
        receivedOrdinal: 1,
        epochDec: '9',
        groupContextDigestHex: '44'.repeat(32),
        verifiedLeafDigestHex: '55'.repeat(32),
        senderLeafIndex: 7,
        senderIdentityHex: '66'.repeat(32),
        senderSignatureKeyHex: '77'.repeat(32),
        senderIdentityProofHex: '88'.repeat(104),
        plaintextBytes,
        plaintextDigestHex: digestHex(plaintextBytes),
      };
      expect(buildInbound(accepted)).toEqual(expect.objectContaining(accepted));
      for (const [field, value] of [
        ['senderLeafIndex', -1],
        ['senderLeafIndex', 0x100000000],
        ['senderIdentityHex', '66'.repeat(31)],
        ['senderSignatureKeyHex', '77'.repeat(33)],
        ['senderIdentityProofHex', '88'.repeat(103)],
        ['epochDec', '-1'],
        ['groupContextDigestHex', '44'.repeat(31)],
        ['verifiedLeafDigestHex', '55'.repeat(33)],
        ['plaintextDigestHex', '99'.repeat(32)],
      ]) {
        expect(() => buildInbound({ ...accepted, [field]: value })).toThrow();
      }
      const missing = { ...accepted };
      delete missing.senderIdentityProofHex;
      expect(() => buildInbound(missing)).toThrow(/non-canonical field set/);
      expect(() => buildInbound({ ...accepted, callerClaimedIdentityHex: 'aa'.repeat(32) }))
        .toThrow(/non-canonical field set/);
      for (const disposition of ['DEFERRED', 'INVALIDATED', 'REJECTED', 'STALE']) {
        expect(() => buildInbound({ ...accepted, disposition, receivedOrdinal: 0,
          plaintextBytes: new Uint8Array(), plaintextDigestHex: null }))
          .toThrow(/non-accepted inbound record contains delivery or sender authority/);
      }
    });
  ...
  test('six outbound messages survive restart and are delivered exactly once', async () => {
    ...
    try {
      ...
      const expectedAlice = rosterMember(
        fixture.peers.bob.group, fixture.peers.alice.publicKey);
      const first = [];
      ...
      for (let index = 0; index < first.length; index += 1) {
        ...
        expectDurableSender(delivered, expectedAlice);
      ...
      expectDurableSender(replay, expectedAlice);
      ...
      await expect(alice.coordinator.queueApplicationMessage(
        ...
        .rejects.toMatchObject({ code: B27_ERROR.REQUEST_CONFLICT });
    } finally { fixture.cleanup(); }
  }, 20000);

  test('three interleaved senders retain exact attribution across out-of-order delivery and restart',
    async () => {
      const wasm = await loadWasm();
      const fixture = await setupGroup(wasm);
      try {
        let receiver = clientFor(
          wasm, fixture.peers.diana, fixture.groupId, 'attribution-interleaved');
        await receiver.initialize();
        const groupIdHex = bytesToHex(fixture.groupId);
        const messages = [];
        for (const name of ['alice', 'bob', 'charlie']) {
          const peer = fixture.peers[name];
          const sender = restoreSource(wasm, peer, fixture.groupId, peer.snapshotBytes);
          const expected = rosterMember(fixture.peers.diana.group, peer.publicKey);
          try {
            for (let generation = 1; generation <= 2; generation += 1) {
              const falseIdentity = bytesToHex(
                fixture.peers[name === 'alice' ? 'bob' : 'alice'].publicKey);
              const plaintext = UTF8.encode(JSON.stringify({
                claimedSenderIdentityHex: falseIdentity,
                actualTestSender: name,
                generation,
              }));
              messages.push({ name, generation, expected, plaintext,
                ciphertext: copyBytes(sender.group.create_application_message(
        ...
        const order = [1, 2, 5, 0, 4, 3];
        for (let index = 0; index < order.length; index += 1) {
          if (index === 3) receiver = restartClient(wasm, receiver);
          const message = messages[order[index]];
          const delivered = await receiver.coordinator.processApplicationMessage(
            groupIdHex, message.ciphertext);
          expectDurableSender(delivered, message.expected);
          expect(delivered.plaintextBytes).toEqual(message.plaintext);
          expect(JSON.parse(new TextDecoder().decode(delivered.plaintextBytes)))
            .toEqual(expect.objectContaining({
              actualTestSender: message.name,
              generation: message.generation,
            }));
        }
        receiver = restartClient(wasm, receiver);
        const replay = await receiver.coordinator.processApplicationMessage(
          groupIdHex, messages[2].ciphertext);
        expect(replay.status).toBe('duplicate');
        expectDurableSender(replay, messages[2].expected);
        expect(replay.plaintextBytes).toEqual(messages[2].plaintext);
      } finally { fixture.cleanup(); }
    }, 20000);

  test.each([
    ['out-of-roster leaf', 'sender_leaf_index', () => 0xffffffff],
    ['credential identity mismatch', 'sender_credential_identity',
      () => new Uint8Array(32).fill(0xa1)],
    ['signature-key mismatch', 'sender_signature_key',
      () => new Uint8Array(32).fill(0xb2)],
    ['epoch mismatch', 'epoch', (received) => received.epoch() + 1n],
    ['group mismatch', 'group_id', () => new Uint8Array(32).fill(0xc3)],
  ])('%s exposes no authority and does not durably burn a valid generation',
    async (_label, field, replacement) => {
      const wasm = await loadWasm();
      const fixture = await setupGroup(wasm);
      try {
        const sender = restoreSource(
          wasm, fixture.peers.alice, fixture.groupId, fixture.peers.alice.snapshotBytes);
        const ciphertexts = [1, 2].map((generation) => copyBytes(
          sender.group.create_application_message(
            sender.provider, sender.identity, UTF8.encode(`sender-bound-${generation}`))));
        free(sender.group); free(sender.identity); free(sender.provider);
        const overriddenWasm = wasmWithReceiveOverride(wasm, (received) =>
          overrideReceivedField(received, field, replacement));
        let receiver = clientFor(overriddenWasm, fixture.peers.bob, fixture.groupId,
          `sender-bound-${field}`);
        await receiver.initialize();
        const before = durableImage(receiver.db);
        await expect(receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), ciphertexts[0]))
          .rejects.toMatchObject({ code: B27_ERROR.ENGINE_REJECTED });
        expect(durableImage(receiver.db)).toEqual(before);
        receiver = restartClient(wasm, receiver);
        const expectedAlice = rosterMember(
          fixture.peers.bob.group, fixture.peers.alice.publicKey);
        const recovered = await receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), ciphertexts[0]);
        expectDurableSender(recovered, expectedAlice);
        const later = await receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), ciphertexts[1]);
        expectDurableSender(later, expectedAlice);
        expect(new TextDecoder().decode(later.plaintextBytes)).toBe('sender-bound-2');
      } finally { fixture.cleanup(); }
    }, 20000);

  test('tampered ciphertext leaves no durable authority and the original generation remains usable',
    async () => {
      const wasm = await loadWasm();
      const fixture = await setupGroup(wasm);
      try {
        const receiver = clientFor(
          wasm, fixture.peers.bob, fixture.groupId, 'tampered-disposable-provider');
        await receiver.initialize();
        const sender = restoreSource(
          wasm, fixture.peers.alice, fixture.groupId, fixture.peers.alice.snapshotBytes);
        const original = copyBytes(sender.group.create_application_message(
          sender.provider, sender.identity, UTF8.encode('original-generation')));
        const later = copyBytes(sender.group.create_application_message(
          sender.provider, sender.identity, UTF8.encode('later-generation')));
        free(sender.group); free(sender.identity); free(sender.provider);
        const tampered = copyBytes(original);
        tampered[tampered.length - 1] ^= 1;
        const before = durableImage(receiver.db);
        await expect(receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), tampered)).rejects.toBeDefined();
        expect(durableImage(receiver.db)).toEqual(before);
        const expectedAlice = rosterMember(
          fixture.peers.bob.group, fixture.peers.alice.publicKey);
        const recovered = await receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), original);
        expectDurableSender(recovered, expectedAlice);
        expect(new TextDecoder().decode(recovered.plaintextBytes))
          .toBe('original-generation');
        const subsequent = await receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), later);
        expectDurableSender(subsequent, expectedAlice);
        expect(new TextDecoder().decode(subsequent.plaintextBytes))
          .toBe('later-generation');
      } finally { fixture.cleanup(); }
    }, 20000);

  test('a producing-edge roster mismatch fails closed before durable sender attribution',
    async () => {
      const wasm = await loadWasm();
      const fixture = await setupGroup(wasm);
      try {
        const receiver = clientFor(
          wasm, fixture.peers.bob, fixture.groupId, 'producing-roster-mismatch');
        await receiver.initialize();
        const groupIdHex = bytesToHex(fixture.groupId);
        const generation = await receiver.coordinator.prepareSelfUpdate(groupIdHex);
        await receiver.coordinator.recordAttempt(groupIdHex);
        await receiver.coordinator.recordAcknowledgement(
          groupIdHex, 1, bytesToHex(fixture.peers.alice.publicKey), UTF8.encode('ack'));
        await receiver.coordinator.settlePass(groupIdHex);
        const sender = restoreSource(
          wasm, fixture.peers.alice, fixture.groupId, fixture.peers.alice.snapshotBytes);
        mergeInbound(sender.group, sender.provider, generation.commitBytes);
        const ciphertext = copyBytes(sender.group.create_application_message(
          sender.provider, sender.identity, UTF8.encode('edge-bound-sender')));
        const expectedAlice = rosterMember(sender.group, fixture.peers.alice.publicKey);
        free(sender.group); free(sender.identity); free(sender.provider);
        const edgeKeys = await receiver.db.transaction(
          [B27_STORES.edge], (ops) => ops.list(B27_STORES.edge));
        expect(edgeKeys).toHaveLength(1);
        const [edgeKey] = edgeKeys;
        const originalEdge = await receiver.db.transaction(
          [B27_STORES.edge], (ops) => ops.get(B27_STORES.edge, edgeKey));
        const parsed = parseEdge(originalEdge);
        expect(parsed.commitDigestHex).toBe(generation.commitDigestHex);
        const conflicting = rebuildEdge(parsed, {
          verifiedLeafDigestHex: parsed.verifiedLeafDigestHex === 'ff'.repeat(32)
            ? 'ee'.repeat(32) : 'ff'.repeat(32),
        });
        await receiver.db.transaction([B27_STORES.edge],
          (ops) => ops.put(B27_STORES.edge, edgeKey, conflicting));
        const before = durableImage(receiver.db);
        await expect(receiver.coordinator.processApplicationMessage(
          groupIdHex, ciphertext)).rejects.toMatchObject({
          code: B27_ERROR.ENGINE_REJECTED,
        });
        expect(durableImage(receiver.db)).toEqual(before);
        await receiver.db.transaction([B27_STORES.edge],
          (ops) => ops.put(B27_STORES.edge, edgeKey, originalEdge));
        const accepted = await receiver.coordinator.processApplicationMessage(
          groupIdHex, ciphertext);
        expectDurableSender(accepted, expectedAlice);
      } finally { fixture.cleanup(); }
    }, 20000);

  test('a corrupted durable sender record is rejected instead of becoming duplicate authority',
    async () => {
      const wasm = await loadWasm();
      const fixture = await setupGroup(wasm);
      try {
        const receiver = clientFor(
          wasm, fixture.peers.bob, fixture.groupId, 'corrupt-durable-sender');
        await receiver.initialize();
        const sender = restoreSource(
          wasm, fixture.peers.alice, fixture.groupId, fixture.peers.alice.snapshotBytes);
        const ciphertext = copyBytes(sender.group.create_application_message(
          sender.provider, sender.identity, UTF8.encode('durable-sender-record')));
        free(sender.group); free(sender.identity); free(sender.provider);
        const accepted = await receiver.coordinator.processApplicationMessage(
        ...
        const expectedAlice = rosterMember(
          fixture.peers.bob.group, fixture.peers.alice.publicKey);
        expectDurableSender(accepted, expectedAlice);
        const inboundKeys = await receiver.db.transaction(
          [B27_STORES.appInbound], (ops) => ops.list(B27_STORES.appInbound));
        expect(inboundKeys).toHaveLength(1);
        const [inboundKeyValue] = inboundKeys;
        const original = await receiver.db.transaction(
          [B27_STORES.appInbound],
          (ops) => ops.get(B27_STORES.appInbound, inboundKeyValue));
        const corrupted = deepClone(original);
        corrupted.senderIdentityHex = 'dd'.repeat(32);
        await receiver.db.transaction([B27_STORES.appInbound],
          (ops) => ops.put(B27_STORES.appInbound, inboundKeyValue, corrupted));
        const before = durableImage(receiver.db);
        await expect(receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), ciphertext)).rejects.toMatchObject({
          code: B27_ERROR.CORRUPT,
        });
        expect(durableImage(receiver.db)).toEqual(before);
        await receiver.db.transaction([B27_STORES.appInbound],
          (ops) => ops.put(B27_STORES.appInbound, inboundKeyValue, original));
        const duplicate = await receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), ciphertext);
        expect(duplicate.status).toBe('duplicate');
        expectDurableSender(duplicate, expectedAlice);
      } finally { fixture.cleanup(); }
    }, 20000);

  test('remove and re-add leaf reuse binds attribution to the new proven leaf key only',
    async () => {
      const wasm = await loadWasm();
      const fixture = await setupLeafReuseGroup(wasm);
      try {
        expect(fixture.readdedMember.senderLeafIndex)
          .toBe(fixture.removedMember.senderLeafIndex);
        expect(fixture.readdedMember.senderIdentityHex)
          .toBe(fixture.removedMember.senderIdentityHex);
        expect(fixture.readdedMember.senderSignatureKeyHex)
          .not.toBe(fixture.removedMember.senderSignatureKeyHex);
        expect(fixture.readdedMember.senderIdentityProofHex)
          .not.toBe(fixture.removedMember.senderIdentityProofHex);
        const receiver = clientFor(
          wasm, fixture.peers.bob, fixture.groupId, 'remove-readd-leaf-reuse');
        await receiver.initialize();
        const oldSender = restoreSource(wasm, fixture.peers.removedCharlie,
          fixture.groupId, fixture.peers.removedCharlie.snapshotBytes);
        const staleCiphertext = copyBytes(oldSender.group.create_application_message(
          oldSender.provider, oldSender.identity, UTF8.encode('removed-leaf')));
        free(oldSender.group); free(oldSender.identity); free(oldSender.provider);
        await expect(receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), staleCiphertext)).rejects.toBeDefined();
        expect(await receiver.db.transaction([B27_STORES.appInbound],
          (ops) => ops.list(B27_STORES.appInbound))).toEqual([]);
        const newSender = restoreSource(wasm, fixture.peers.charlie,
          fixture.groupId, fixture.peers.charlie.snapshotBytes);
        const freshCiphertext = copyBytes(newSender.group.create_application_message(
          newSender.provider, newSender.identity, UTF8.encode('re-added-leaf')));
        free(newSender.group); free(newSender.identity); free(newSender.provider);
        const accepted = await receiver.coordinator.processApplicationMessage(
          bytesToHex(fixture.groupId), freshCiphertext);
        expectDurableSender(accepted, fixture.readdedMember);
        expect(accepted).not.toEqual(expect.objectContaining({
          senderSignatureKeyHex: fixture.removedMember.senderSignatureKeyHex,
          senderIdentityProofHex: fixture.removedMember.senderIdentityProofHex,
        }));
      } finally { fixture.cleanup(); }
    }, 20000);
  ...
  test.each([
    ['before', B27_STORES.messageSnapshot],
    ['before', B27_STORES.messageState],
    ['before', B27_STORES.appOutbox],
    ['after', B27_STORES.messageSnapshot],
    ['after', B27_STORES.messageState],
    ['after', B27_STORES.appOutbox],
    ...
    try {
      ...
      expect(await db.transaction([B27_STORES.messageState],
        (ops) => ops.list(B27_STORES.messageState))).toEqual([]);
      expect(await db.transaction([B27_STORES.messageSnapshot],
        (ops) => ops.list(B27_STORES.messageSnapshot))).toEqual([]);
      expect(await db.transaction([B27_STORES.appOutbox],
        (ops) => ops.list(B27_STORES.appOutbox))).toEqual([]);
      db.failOn = null;
  ...
  test.each([
    ['before', B27_STORES.messageSnapshot],
    ['before', B27_STORES.messageState],
    ['before', B27_STORES.appInbound],
    ['after', B27_STORES.messageSnapshot],
    ['after', B27_STORES.messageState],
    ['after', B27_STORES.appInbound],
    ...
    async (phase, store) => {
      ...
      try {
        ...
        expect(await db.transaction([B27_STORES.messageSnapshot],
          (ops) => ops.list(B27_STORES.messageSnapshot))).toEqual([]);
        expect(await db.transaction([B27_STORES.appInbound],
          (ops) => ops.list(B27_STORES.appInbound))).toEqual([]);
        db.failOn = null;
        db.failAfterStore = null;
  ...
  test('concurrent senders have one message-position CAS winner', async () => {
    ...
    try {
      ...
      expect(outcomes.find((item) => item.status === 'rejected').reason)
        .toMatchObject({ code: B27_ERROR.CAS_CONFLICT });
      const stateKeys = await client.db.transaction([B27_STORES.messageState],
        (ops) => ops.list(B27_STORES.messageState));
      const outboxKeys = await client.db.transaction([B27_STORES.appOutbox],
        (ops) => ops.list(B27_STORES.appOutbox));
  ...
  test('a liveness probe and ordinary send cannot release the same message position',
    async () => {
      ...
      try {
        ...
        expect(await client.db.transaction([B27_STORES.appOutbox],
          (ops) => ops.list(B27_STORES.appOutbox))).toHaveLength(1);
      } finally { fixture.cleanup(); }
    }, 20000);
    ...
    try {
      ...
      expect(outcomes.filter((item) => item.status === 'rejected')).toEqual([
        expect.objectContaining({ reason: expect.objectContaining({
          code: B27_ERROR.CAS_CONFLICT,
        }) }),
      ]);
      expect(await client.db.transaction([B27_STORES.appInbound],
        (ops) => ops.list(B27_STORES.appInbound))).toHaveLength(1);
    ...
    async () => {
      ...
      try {
        ...
        await expect(alice.coordinator.readQueuedApplicationMessage(
          ...
          code: B27_ERROR.OUTBOX_TERMINAL,
        });
  ...
  test('message codecs reject sender authority on non-accepted records and bounds fail before mutation', async () => {
    const wasm = await loadWasm();
    const fixture = await setupGroup(wasm);
    try {
      ...
      await expect(client.coordinator.queueApplicationMessage(
        groupIdHex, 'x'.repeat(B27_LIMITS.maxRequestIdBytes + 1), UTF8.encode('x')))
        .rejects.toMatchObject({ code: expect.stringMatching(/_INVALID$/) });
      await expect(client.coordinator.queueApplicationMessage(
        groupIdHex, `styx-probe:${'11'.repeat(32)}`, UTF8.encode('x')))
        .rejects.toMatchObject({ code: B27_ERROR.INVALID });
      await expect(client.coordinator.queueApplicationMessage(
        groupIdHex, 'oversized', new Uint8Array(B27_LIMITS.maxApplicationPayloadBytes + 1)))
        .rejects.toBeDefined();
      expect(await client.db.transaction([B27_STORES.messageState],
        (ops) => ops.list(B27_STORES.messageState))).toEqual([]);
      for (let index = 0; index < B27_LIMITS.maxOutboxPerInstance; index += 1) {
      ...
      await expect(client.coordinator.queueApplicationMessage(
        ...
        .rejects.toMatchObject({ code: B27_ERROR.RESOURCE_LIMIT });
      ...
      expect(await client.db.transaction([B27_STORES.appOutbox],
        (ops) => ops.list(B27_STORES.appOutbox)))
        .toHaveLength(B27_LIMITS.maxOutboxPerInstance);
    } finally { fixture.cleanup(); }
  }, 20000);
    ...
    async () => {
      ...
      try {
        ...
        const sender = restoreSource(
          wasm, fixture.peers.alice, fixture.groupId, fixture.peers.alice.snapshotBytes);
        const ciphertexts = [];
        for (let index = 0; index <= B27_LIMITS.maxInboundPerInstance; index += 1) {
        ...
        const inboundKeys = await bob.db.transaction([B27_STORES.appInbound],
          (ops) => ops.list(B27_STORES.appInbound));
        expect(inboundKeys).toHaveLength(B27_LIMITS.maxInboundPerInstance);
        const marker = await bob.db.transaction([B27_STORES.inboundTruncation],
          (ops) => ops.get(B27_STORES.inboundTruncation, context.instanceKeyHex));
    ...
    async () => {
      ...
      try {
        ...
        for (const ciphertext of ciphertexts) {
          ...
          expectNoSenderAuthority(deferred);
        }
        ...
        expectDurableSender(accepted, rosterMember(
          fixture.peers.bob.group, fixture.peers.alice.publicKey));
        const inboundKeys = await client.db.transaction([B27_STORES.appInbound],
          (ops) => ops.list(B27_STORES.appInbound));
        expect(inboundKeys).toHaveLength(B27_LIMITS.maxInboundPerInstance);
        expect(client.db.stores.get(B27_STORES.inboundTruncation)?.size ?? 0).toBe(0);
      } finally { fixture.cleanup(); }
    }, 20000);
  ...
  test.each([
    ...
    try {
      ...
      expect(head.state).toBe(B27_HEAD_STATE.STABLE);
  ...
  test('pinned out-of-order window accepts skipped messages and rejects excessive gaps',
    async () => {
      ...
      try {
        ...
        expect(await charlie.db.transaction([B27_STORES.appInbound],
          (ops) => ops.list(B27_STORES.appInbound))).toEqual([]);
      } finally { fixture.cleanup(); }
    }, 20000);
  ...
  test('write-ahead probe is one-use across restart and remains selection-inert', async () => {
    ...
    try {
      ...
      await expect(bob.coordinator.createLivenessProbe(
        ...
        code: B27_ERROR.PROBE_ALREADY_RESERVED,
      });
      const restarted = createB27Coordinator({ journal: bob.journal, wasm });
  ...
  test('a crash after durable reservation produces refusal without completion', async () => {
    ...
    try {
      ...
      expect(client.db.stores.get(B27_STORES.messageState)?.size ?? 0).toBe(0);
      expect(client.db.stores.get(B27_STORES.messageSnapshot)?.size ?? 0).toBe(0);
      expect(client.db.stores.get(B27_STORES.appOutbox)?.size ?? 0).toBe(0);
      expect((await client.journal.readHead(bytesToHex(fixture.groupId))).head.headDigestHex)
        .toBe(headBefore.headDigestHex);
      const restarted = createB27Coordinator({ journal: client.journal, wasm });
      await expect(restarted.createLivenessProbe(
  ...
  test('discarded local propagation contradiction terminalizes without changing canonical state',
    async () => {
      ...
      try {
        ...
        for (let index = 0; index < B27_LIMITS.maxGenerations; index += 1) {
        ...
        expect(bounded.generations).toHaveLength(B27_LIMITS.maxGenerations);
        ...
        await expect(client.coordinator.settle(pass.passDigestHex)).rejects.toMatchObject({
          code: B27_ERROR.UNRECOVERABLE,
        });
        const terminal = (await client.journal.readHead(groupIdHex)).head;
        expect(terminal.state).toBe(B27_HEAD_STATE.UNRECOVERABLE);
  ...
  test.each([
    ...
    B27_STORES.input,
    B27_STORES.pass,
    B27_STORES.invalidation,
    B27_STORES.transition,
    B27_STORES.head,
  ...
  test('new input admitted after the decision snapshot forces complete CAS retry', async () => {
    ...
    try {
      ...
      release();
      await expect(pending).rejects.toMatchObject({ code: B27_ERROR.CAS_CONFLICT });
    ...
    async () => {
      ...
      try {
        ...
        await expect(client.journal.readRetained(initial.snapshotDigestHex)).rejects
          .toMatchObject({ code: B27_ERROR.RELEASED });
        await expect(client.journal.readRetainedMessageContext(
          groupIdHex, initial.snapshotDigestHex)).rejects
          .toMatchObject({ code: B27_ERROR.RELEASED });
        const snapshot = await client.journal.snapshot(groupIdHex);
        expect(snapshot.edges.length).toBeLessThanOrEqual(B27_LIMITS.maxEdges);
        expect(snapshot.retained.length).toBeLessThanOrEqual(B27_LIMITS.maxStates);
        ...
        expect(client.db.stores.get(B27_STORES.messageRelease)?.size).toBe(1);
      } finally { fixture.cleanup(); }
    }, 20000);
  ...
  test('local generation can provide the replay-authorized anchor-advance edge', async () => {
    ...
    try {
      ...
      const chain = selfUpdateChainFrom(
        ...
        B27_LIMITS.rewindCommits);
      const settled = await settleInputs(client, fixture.groupId,
        chain.map((item) => item.commitBytes).reverse());

      expect(settled.head.canonicalPath).toHaveLength(B27_LIMITS.rewindCommits);
  ...
  test('contradictory adopted-anchor binding evidence fails closed without mutation',
    async () => {
      ...
      try {
        ...
        client.db.stores.get(B27_STORES.edge)
  ...
  test('record-first release rejects a stored instance-key contradiction without mutation',
    async () => {
      ...
      try {
        ...
        const original = client.db.stores.get(B27_STORES.messageState).get(stateKey);
        ...
        client.db.stores.get(B27_STORES.messageState).delete(stateKey);
        client.db.stores.get(B27_STORES.messageState).set(
  ...
  test('branch-changing anchor re-adoption preserves a prior-head durable ratchet',
    async () => {
      ...
      try {
        ...
        expect(client.db.stores.get(B27_STORES.messageRelease)
  ...
  test('single and split cutoffs converge when anchor advancement also discards a new sibling',
    async () => {
      ...
      try {
        ...
        expect(singleHead.canonicalPath).toHaveLength(B27_LIMITS.rewindCommits);
  ...
  test('a superseded probed branch can return only through a deeper fresh tip', async () => {
    ...
    try {
      ...
      expect(client.db.stores.get(B27_STORES.probeReservation).size).toBe(2);
  ...
  test('historical canonical input is accepted while sibling input waits for re-adoption',
    async () => {
      ...
      try {
        ...
        expectDurableSender(historical, rosterMember(
          fixture.peers.bob.group, fixture.peers.alice.publicKey));
    ...
    async () => {
      ...
      try {
        ...
        const deferred = await client.coordinator.processApplicationMessage(
          groupIdHex, ciphertext, displacedEdge.successorSnapshotDigestHex);
        expect(deferred.status).toBe('deferred');
        expect(client.db.stores.get(B27_STORES.messageState)
    ...
    async () => {
      ...
      try {
        ...
        await expect(client.coordinator.processApplicationMessage(
          ...
          .rejects.toMatchObject({ code: B27_ERROR.RELEASED });
        expect(client.db.stores.get(B27_STORES.appInbound)?.size ?? 0).toBe(0);
        expect(client.db.stores.get(B27_STORES.retained)
          .has(generation.pendingSnapshotDigestHex)).toBe(false);
        expect(client.db.stores.get(B27_STORES.released)
  ...
  test('message-state instance cap fails before ratchet mutation', async () => {
    ...
    try {
      ...
      const groupIdHex = bytesToHex(fixture.groupId);
      const context = await client.journal.readMessageContext(groupIdHex);
      const store = client.db.stores.get(B27_STORES.messageState);
      for (let index = 0; index < B27_LIMITS.maxMessageStates; index += 1) {
      ...
      expect(client.db.stores.get(B27_STORES.appOutbox).size).toBe(0);
    } finally { fixture.cleanup(); }
  }, 20000);
  ...
  test('bounded generation history evicts terminal evidence with a durable marker',
    async () => {
      ...
      try {
        ...
        const groupIdHex = bytesToHex(fixture.groupId);
        let first;
        for (let index = 0; index < B27_LIMITS.maxGenerations + 1; index += 1) {
          const generation = await client.coordinator.prepareSelfUpdate(groupIdHex);
          first ??= generation;
          await client.coordinator.cancelBeforeAttempt(groupIdHex);
        }
        const snapshot = await client.journal.snapshot(groupIdHex);
        expect(snapshot.generations).toHaveLength(B27_LIMITS.maxGenerations);
        ...
        await expect(client.coordinator.recordHistoricalAcknowledgement(
          ...
          .rejects.toMatchObject({
            code: B27_ERROR.UNKNOWN_GENERATION,
  ...
  test('strict input codec and raw-input cap fail before mutation', async () => {
    ...
    try {
      ...
      for (let index = 0; index < B27_LIMITS.maxInputs; index += 1) {
  ...
  test('a displaced instance suspends exact-byte publication and resumes after re-adoption',
    async () => {
      ...
      try {
        ...
        await expect(client.coordinator.readQueuedApplicationMessage(
          queued.instanceKeyHex, queued.ordinal)).rejects.toMatchObject({
          code: B27_ERROR.OUTBOX_SUSPENDED,
        });
  ...
  test('journal factory rejects databases outside the isolated B2.7 namespace', () => {
    const db = new FakeVaultDb();
    Object.defineProperty(db, 'name', { value: 'styx-unscoped-database' });
    expect(() => createB27JournalForDb(db)).toThrow(/outside the B2\.7 namespace/);
    const previousPhase = new FakeVaultDb();
    Object.defineProperty(previousPhase, 'name', { value: 'styx-b2-6-poc-v2-retired' });
    expect(() => createB27JournalForDb(previousPhase))
      .toThrow(/outside the B2\.7 namespace/);
  });
  ...
  test('head reads reject a missing transition audit record without mutation', async () => {
    ...
    try {
      ...
      const groupIdHex = bytesToHex(fixture.groupId);
      const head = (await client.journal.readHead(groupIdHex)).head;
      client.db.stores.get(B27_STORES.transition).delete(head.transitionDigestHex);
      const before = durableImage(client.db);
  ...
  test('message readers reject a state bound to a different retained base', async () => {
    ...
    try {
      ...
      const original = client.db.stores.get(B27_STORES.messageState).get(key);
      const { format, version, profile, runtime, stateDigestHex, ...stateFields } = original;
      const contradictory = buildMessageState({
        ...stateFields,
        baseRetainedSnapshotDigestHex: 'fd'.repeat(32),
      });
      client.db.stores.get(B27_STORES.messageState).set(key, contradictory);
      const before = durableImage(client.db);
    ...
    async () => {
      ...
      try {
        ...
        for (const successor of [first, second]) {
          ...
          client.db.stores.get(B27_STORES.retained)
  ...
  test('missing required retained anchor terminalizes the group without moving its state',
    async () => {
      ...
      try {
        ...
        const groupIdHex = bytesToHex(fixture.groupId);
        const before = (await client.journal.readHead(groupIdHex)).head;
        client.db.stores.get(B27_STORES.retained).delete(before.anchorSnapshotDigestHex);
        ...
        expect(after.state).toBe(B27_HEAD_STATE.UNRECOVERABLE);
