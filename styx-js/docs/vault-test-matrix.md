...
and why. US-009 adds the first product migration (`settings`) while later
...
| 3 | migration (happy path, crash at each §10 step, localStorage intact) | covered | `test/storage/vault-migration.test.js` proves the exact two-key allowlist, strict normalization and canonical bytes. `test/storage/vault.test.js` injects failure before/during the record transaction, after the IDB commit, before the verified-marker commit and after verification; every retry preserves `rv` and reaches only `verified`. `apps/chat/test/vault-settings.test.js` covers failures before/after legacy reads and between legacy-first and vault writes. `test/storage/vault-settings.browser.spec.js` proves real-worker persistence/reload on Chromium and Firefox; `apps/chat/e2e/vault-settings.spec.js` proves the stage-enabled product integration and that legacy/unrelated data remains intact. Cleanup states are intentionally not part of US-009 |
...
| 6 | quota (QuotaExceeded on PUT and on migration → fail-closed, non-destructive) | partial | Quota mapping and transaction rollback are covered deterministically in `test/storage/vault-db.test.js`; migration transaction-failure/retry and legacy preservation are covered in `test/storage/vault.test.js` and `apps/chat/test/vault-settings.test.js`. The real browser quota probe remains P8 in `test/storage/vault-db.browser.spec.js`, which skips honestly where the experimental CDP override is not enforced (manual item M3) |
...
| 16 | real browsers (Playwright chromium + firefox) | covered | The `Vault browser probes` CI job runs all five root configs on both engines, including `playwright.vault-settings.config.js`; the stage-enabled PWA integration additionally runs `apps/chat/playwright.vault-settings.config.js` on Chromium. The WASM integrity workflow separately runs its browser KDF checks |
...
the US-008 worker integration and the US-009 settings increment is automated on
desktop browser engines; settings are the only real product namespace enabled.
