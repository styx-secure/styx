...
and why. US-009 added the first product migration (`settings`); US-010 adds the
reversible identity shadow migration while legacy remains authoritative. Later
namespaces and real mobile devices remain outside this automated gate.
...
| 3 | migration (happy path, crash at each §10 step, localStorage intact) | covered | `test/storage/vault-migration.test.js` proves the exact settings allowlist and exact single identity key, strict schemas, canonical Base64 and canonical bytes. `test/storage/vault.test.js` injects failures at the pending, record/written and verified phases for both product records; retry preserves or monotonically repairs `rv` and reaches only `verified`. `apps/chat/test/vault-settings.test.js` and `apps/chat/test/vault-identity.test.js` cover page-boundary failure, divergence and value-free fallback. `test/storage/vault-settings.browser.spec.js` and `test/storage/vault-identity.browser.spec.js` prove real-worker persistence/restart on Chromium and Firefox. The two app E2E suites prove stage-enabled integration, unchanged legacy bytes, profile isolation, deferred transport, and the persisted-pairing no-write gate. Cleanup/read cutover is intentionally outside US-009/US-010 |
...
| 6 | quota (QuotaExceeded on PUT and on migration → fail-closed, non-destructive) | partial | Quota mapping and transaction rollback are covered deterministically in `test/storage/vault-db.test.js`; settings and identity migration transaction-failure/retry plus legacy authority are covered in `test/storage/vault.test.js` and the app coordinators. The real browser quota probe remains P8 in `test/storage/vault-db.browser.spec.js`, which skips honestly where the experimental CDP override is not enforced (manual item M3) |
...
| 16 | real browsers (Playwright chromium + firefox) | covered | The `Vault browser probes` CI job runs all six root configs on both engines, including the settings and identity migration probes. The stage-enabled PWA integrations run the settings and identity app configs on Chromium. The WASM integrity workflow separately runs its browser KDF checks |
...
Row 17 remains manual: real mobile devices are manual by definition. Row 14 remains partial only for reset steps
whose product subsystems do not exist in this canary block. Everything owned by
the US-008 worker integration, US-009 settings increment and US-010 identity
shadow migration is automated on desktop browser engines. `settings` and
`identity` are the only real product namespaces enabled; neither is a runtime
