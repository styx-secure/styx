...
| 11 | AAD tampering (record copied to another key/namespace/version → auth fail) | covered | `test/storage/vault-canary.test.js` (an intact envelope moved to another key fails; the original stays readable) |
...
| 14 | factory reset (after every state, §12 order) | partial | The §12 ordering (wipe keys → overwrite wrapper → delete database) and reset from UNLOCKED, LOCKED and a malformed/ERROR vault are covered in `test/storage/vault-canary.test.js` and `test/storage/vault.test.js`. Steps 4–7 of §12 (legacy localStorage, Cache Storage, push subscription, outbox) have no subject yet in this block; step 8 (worker terminate/respawn) is deferred to US-008 |
...
| 16 | real browsers (Playwright chromium + firefox) | covered | Every `*.browser.spec.js` suite runs on both engines in the `Vault engine browser probes` CI job |
