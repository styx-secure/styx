// STYX_SPIKE_PROTOTYPE — explicit, clock-free B2.7 orchestration boundary.

import { B27_ERROR, failB27 } from './b2-7-canonical.mjs';

function removePayload(leafIndex) {
  if (!Number.isSafeInteger(leafIndex) || leafIndex < 0 || leafIndex > 0xffffffff) {
    failB27(B27_ERROR.INVALID, 'removed leaf index must be one u32');
  }
...
export class B27Coordinator {
  ...
  constructor({ journal, wasm, beforeReplay = async () => {},
    ...
    if (!journal || typeof journal.createEngineAdapter !== 'function') {
      failB27(B27_ERROR.INVALID, 'coordinator requires isolated B2.7 journal');
    }
...
export function createB27Coordinator(options) {
  return new B27Coordinator(options);
}
