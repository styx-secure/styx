// STYX_SPIKE_PROTOTYPE — atomic retained-graph journal for Phase B2.7 only.

import { openVaultDb } from '../../src/storage/vault-db.js';
import {
  B27_DB_PREFIX,
  B27_APP_PUBLICATION_KIND,
  B27_ERROR,
  B27_GENERATION_STATE,
  B27_HEAD_STATE,
  B27_INPUT_STATE,
  B27_INBOUND_STATE,
  B27_LIMITS,
  B27_MESSAGE_STATE,
  B27_OUTBOX_STATE,
  B27_PASS_STATE,
  B27_STORES,
  B27_STORE_NAMES,
  assertBytes,
  assertGroupIdHex,
  assertHex64,
  assertSafeInteger,
  assertString,
  bytesEqual,
  canonicalB27Bytes,
  digestHex,
  edgeKey,
  failB27,
  generationKey,
  generationAuthorityDigest,
  inboundKey,
  inputKey,
  messageInstanceKey,
  messageSnapshotKey,
  messageStateKey,
  outboxKey,
  probeKey,
  publicationKey,
} from './b2-7-canonical.mjs';
import {
  GENERATION_FIELDS,
  INBOUND_FIELDS,
  MESSAGE_STATE_FIELDS,
  OUTBOX_FIELDS,
  buildGenerationTruncation,
  INPUT_FIELDS,
  buildActiveLocal,
  buildGeneration,
  buildHead,
  buildAppPublication,
  buildInbound,
  buildInboundTruncation,
  buildMessageRelease,
  buildMessageSnapshot,
  buildMessageState,
  buildOutbox,
  buildInput,
  buildPass,
  buildProbeCompletion,
  buildProbeReservation,
  buildRelease,
  buildRetainedState,
  buildTransition,
  parseActiveLocal,
  parseEdge,
  parseGeneration,
  parseGenerationTruncation,
  parseHead,
  parseAppPublication,
  parseInbound,
  parseInboundTruncation,
  parseInput,
  parseInvalidation,
  parseMessageRelease,
  parseMessageSnapshot,
  parseMessageState,
  parseOutbox,
  parsePass,
  parseProbeCompletion,
  parseProbeReservation,
  parsePublication,
  parseRelease,
  parseRetainedState,
  parseTransition,
} from './b2-7-record.mjs';
import { createBoundB27EngineAdapter } from './b2-7-engine-adapter.mjs';

const JOURNAL_TOKEN = Symbol('B27_JOURNAL_TOKEN');
const SNAPSHOT_STORES = Object.freeze([
  B27_STORES.head, B27_STORES.retained, B27_STORES.released,
  B27_STORES.input, B27_STORES.edge, B27_STORES.pass,
  B27_STORES.transition, B27_STORES.invalidation, B27_STORES.generation,
  B27_STORES.generationTruncation, B27_STORES.activeLocal, B27_STORES.publication,
]);
const MESSAGE_TRANSACTION_STORES = Object.freeze([
  B27_STORES.head, B27_STORES.retained, B27_STORES.released,
  B27_STORES.edge, B27_STORES.transition, B27_STORES.generation,
  B27_STORES.messageState, B27_STORES.messageSnapshot, B27_STORES.appOutbox,
  B27_STORES.appInbound, B27_STORES.appPublication, B27_STORES.messageRelease,
  B27_STORES.inboundTruncation,
]);
const SETTLEMENT_TRANSACTION_STORES = Object.freeze(
  [...new Set([...SNAPSHOT_STORES, ...MESSAGE_TRANSACTION_STORES])]);
const TIP_EVIDENCE_GENERATION_STATES = new Set([
  B27_GENERATION_STATE.PREPARED,
  B27_GENERATION_STATE.PUBLISHING,
  B27_GENERATION_STATE.ACKNOWLEDGED,
  B27_GENERATION_STATE.SELECTED,
  B27_GENERATION_STATE.LOSING,
]);

function migrationV1(db) {
  for (const name of B27_STORE_NAMES) db.createObjectStore(name);
}

async function required(ops, store, key, label) {
  const value = await ops.get(store, key);
  if (value === undefined) failB27(B27_ERROR.CORRUPT, `${label} is missing`);
  return value;
}

async function groupRecords(ops, store, parser, groupIdHex, label) {
  const records = [];
  for (const key of await ops.list(store)) {
    if (typeof key !== 'string') continue;
    const record = parser(await required(ops, store, key, label));
    if (record.groupIdHex === groupIdHex) records.push(record);
  }
  return records;
}

function resolveTipCommitDigestHex({
  snapshotDigestHex, head, edges, generations,
}) {
  const candidates = new Set();
  const add = (candidate) => {
    if (candidate !== null && candidate !== undefined) candidates.add(candidate);
  };
  for (const edge of edges) {
    if (edge.successorSnapshotDigestHex === snapshotDigestHex) add(edge.commitDigestHex);
  }
  if (head.anchorSnapshotDigestHex === snapshotDigestHex) {
    add(head.anchorTipCommitDigestHex);
  }
  for (const generation of generations) {
    if (generation.pendingSnapshotDigestHex === snapshotDigestHex
      && TIP_EVIDENCE_GENERATION_STATES.has(generation.state)) {
      add(generation.commitDigestHex);
    }
  }
  if (head.snapshotDigestHex === snapshotDigestHex) add(head.selectedCommitDigestHex);
  if (candidates.size > 1) {
    failB27(B27_ERROR.CORRUPT,
      'retained state has conflicting transcript-derived tip Commits');
  }
  const resolved = candidates.values().next().value;
  if (resolved === undefined) {
    const isGenesisAnchor = snapshotDigestHex === head.anchorSnapshotDigestHex
      && head.anchorTipCommitDigestHex === null;
    if (!isGenesisAnchor) {
      failB27(B27_ERROR.CORRUPT,
        'non-genesis retained state lacks a transcript-derived tip Commit');
    }
    return null;
  }
  return resolved;
}

function producingVerifiedLeafDigestHex(snapshotDigestHex, tipCommitDigestHex, edges) {
  if (tipCommitDigestHex === null) return null;
  const matches = edges.filter((edge) =>
    edge.commitDigestHex === tipCommitDigestHex
      && edge.successorSnapshotDigestHex === snapshotDigestHex);
  if (matches.length > 1) {
    failB27(B27_ERROR.CORRUPT,
      'retained state has multiple producing replay edges');
  }
  return matches[0]?.verifiedLeafDigestHex ?? null;
}

async function readTipAuthority(ops, groupIdHex, snapshotDigestHex, head) {
  const [edges, generations] = await Promise.all([
    groupRecords(ops, B27_STORES.edge, parseEdge, groupIdHex, 'replay edge'),
    groupRecords(ops, B27_STORES.generation, parseGeneration, groupIdHex,
      'local generation'),
  ]);
  const tipCommitDigestHex = resolveTipCommitDigestHex({
    snapshotDigestHex, head, edges, generations,
  });
  return Object.freeze({
    tipCommitDigestHex,
    producingVerifiedLeafDigestHex: producingVerifiedLeafDigestHex(
      snapshotDigestHex, tipCommitDigestHex, edges),
  });
}

async function enforceMessageStateCapacity(ops, groupIdHex, instanceKeyHex) {
  let count = 0;
  for (const key of await ops.list(B27_STORES.messageState)) {
    if (typeof key !== 'string') continue;
    const state = parseMessageState(await required(
      ops, B27_STORES.messageState, key, 'message-state record'));
    if (state.groupIdHex !== groupIdHex) continue;
    if (state.instanceKeyHex === instanceKeyHex) return;
    count += 1;
  }
  if (count >= B27_LIMITS.maxMessageStates) {
    failB27(B27_ERROR.RESOURCE_LIMIT, 'message-state instance cap is exhausted');
  }
}

async function requireLiveRetainedState(ops, expectedRetained) {
  const expected = parseRetainedState(expectedRetained);
  const raw = await ops.get(B27_STORES.retained, expected.snapshotDigestHex);
  if (raw === undefined) {
    const releasedRaw = await ops.get(B27_STORES.released, expected.snapshotDigestHex);
    if (releasedRaw !== undefined) {
      parseRelease(releasedRaw);
      failB27(B27_ERROR.RELEASED,
        'message base left the retained horizon before commit');
    }
    failB27(B27_ERROR.CORRUPT,
      'message base disappeared without durable release evidence');
  }
  const current = parseRetainedState(raw);
  if (current.retainedDigestHex !== expected.retainedDigestHex) {
    failB27(B27_ERROR.CORRUPT,
      'message base changed under its content-addressed key');
  }
  return current;
}

async function releaseMessageInstanceRecords(ops, {
  groupIdHex, instanceKeyHex, releaseAuthorityDigestHex,
}) {
  const writes = [];
  const key = messageStateKey(groupIdHex, instanceKeyHex);
  const stateRaw = await ops.get(B27_STORES.messageState, key);
  if (stateRaw !== undefined) {
    const state = parseMessageState(stateRaw);
    const marker = buildMessageRelease({
      groupIdHex, instanceKeyHex, stateDigestHex: state.stateDigestHex,
      snapshotDigestHex: state.snapshotDigestHex, releaseAuthorityDigestHex,
    });
    writes.push(
      ops.put(B27_STORES.messageRelease, key, marker),
      ops.delete(B27_STORES.messageState, key),
      ops.delete(B27_STORES.messageSnapshot,
        messageSnapshotKey(instanceKeyHex, state.snapshotDigestHex)),
    );
  }
  for (const outboxKeyValue of await ops.list(B27_STORES.appOutbox)) {
    if (typeof outboxKeyValue !== 'string'
      || !outboxKeyValue.startsWith(instanceKeyHex + ':')) continue;
    const outbox = parseOutbox(await required(
      ops, B27_STORES.appOutbox, outboxKeyValue, 'outbox record'));
    if ([B27_OUTBOX_STATE.ACKNOWLEDGED, B27_OUTBOX_STATE.FAILED_DISCARDED,
      B27_OUTBOX_STATE.INVALIDATED].includes(outbox.state)) continue;
    writes.push(ops.put(B27_STORES.appOutbox, outboxKeyValue,
      rebuildOutbox(outbox, { state: B27_OUTBOX_STATE.INVALIDATED })));
  }
  return writes;
}

async function releaseRetainedMessageRecords(ops, {
  head, retained, tipCommitDigestHex, releaseAuthorityDigestHex,
}) {
  const expectedInstanceKeyHex = messageInstanceKey({
    groupIdHex: retained.groupIdHex,
    tipCommitDigestHex,
    epochDec: retained.epochDec,
    groupContextDigestHex: retained.groupContextDigestHex,
    localMemberIdentityHex: head.accountKeyHex,
  });
  const storedInstanceKeys = new Set();
  for (const key of await ops.list(B27_STORES.messageState)) {
    if (typeof key !== 'string') continue;
    const state = parseMessageState(await required(
      ops, B27_STORES.messageState, key, 'message-state record'));
    if (state.groupIdHex !== retained.groupIdHex
      || state.baseRetainedSnapshotDigestHex !== retained.snapshotDigestHex) continue;
    if (key !== messageStateKey(state.groupIdHex, state.instanceKeyHex)
      || state.instanceKeyHex !== expectedInstanceKeyHex) {
      failB27(B27_ERROR.CORRUPT,
        'stored message-state identity contradicts retained transcript evidence');
    }
    storedInstanceKeys.add(state.instanceKeyHex);
  }
  const writes = [];
  for (const instanceKeyHex of storedInstanceKeys) {
    writes.push(...await releaseMessageInstanceRecords(ops, {
      groupIdHex: retained.groupIdHex,
      instanceKeyHex,
      releaseAuthorityDigestHex,
    }));
  }
  writes.push(...await invalidateDeferredInboundForBase(ops, {
    groupIdHex: retained.groupIdHex,
    baseRetainedSnapshotDigestHex: retained.snapshotDigestHex,
  }));
  return writes;
}

async function invalidateDeferredInboundForBase(ops, {
  groupIdHex, baseRetainedSnapshotDigestHex,
}) {
  const writes = [];
  for (const inboundKeyValue of await ops.list(B27_STORES.appInbound)) {
    if (typeof inboundKeyValue !== 'string') continue;
    const inbound = parseInbound(await required(
      ops, B27_STORES.appInbound, inboundKeyValue, 'inbound record'));
    if (inbound.groupIdHex !== groupIdHex
      || inbound.baseRetainedSnapshotDigestHex !== baseRetainedSnapshotDigestHex
      || inbound.disposition !== B27_INBOUND_STATE.DEFERRED) continue;
    writes.push(ops.put(B27_STORES.appInbound, inboundKeyValue,
      rebuildInbound(inbound, {
        disposition: B27_INBOUND_STATE.INVALIDATED,
        receivedOrdinal: 0,
        plaintextBytes: new Uint8Array(),
        plaintextDigestHex: null,
      })));
  }
  return writes;
}

function recordDigest(store, record) {
  const fields = {
    [B27_STORES.head]: 'headDigestHex',
    [B27_STORES.retained]: 'retainedDigestHex',
    [B27_STORES.released]: 'releaseDigestHex',
    [B27_STORES.input]: 'inputDigestHex',
    [B27_STORES.edge]: 'edgeDigestHex',
    [B27_STORES.pass]: 'passRecordDigestHex',
    [B27_STORES.transition]: 'transitionDigestHex',
    [B27_STORES.invalidation]: 'invalidationDigestHex',
    [B27_STORES.generation]: 'generationDigestHex',
    [B27_STORES.generationTruncation]: 'markerDigestHex',
    [B27_STORES.activeLocal]: 'pointerDigestHex',
    [B27_STORES.publication]: 'evidenceDigestHex',
    [B27_STORES.messageState]: 'stateDigestHex',
    [B27_STORES.messageSnapshot]: 'recordDigestHex',
    [B27_STORES.appOutbox]: 'outboxDigestHex',
    [B27_STORES.appInbound]: 'inboundDigestHex',
    [B27_STORES.appPublication]: 'evidenceDigestHex',
    [B27_STORES.messageRelease]: 'releaseDigestHex',
    [B27_STORES.inboundTruncation]: 'markerDigestHex',
  };
  const field = fields[store];
  if (field === undefined || record[field] === undefined) {
    failB27(B27_ERROR.CORRUPT, 'record lacks a content digest');
  }
  return record[field];
}

function parseStoreRecord(store, raw) {
  if (store === B27_STORES.head) return parseHead(raw);
  if (store === B27_STORES.retained) return parseRetainedState(raw);
  if (store === B27_STORES.released) return parseRelease(raw);
  if (store === B27_STORES.input) return parseInput(raw);
  if (store === B27_STORES.edge) return parseEdge(raw);
  if (store === B27_STORES.pass) return parsePass(raw);
  if (store === B27_STORES.transition) return parseTransition(raw);
  if (store === B27_STORES.invalidation) return parseInvalidation(raw);
  if (store === B27_STORES.generation) return parseGeneration(raw);
  if (store === B27_STORES.generationTruncation) return parseGenerationTruncation(raw);
  if (store === B27_STORES.activeLocal) return parseActiveLocal(raw);
  if (store === B27_STORES.publication) return parsePublication(raw);
  if (store === B27_STORES.messageState) return parseMessageState(raw);
  if (store === B27_STORES.messageSnapshot) return parseMessageSnapshot(raw);
  if (store === B27_STORES.appOutbox) return parseOutbox(raw);
  if (store === B27_STORES.appInbound) return parseInbound(raw);
  if (store === B27_STORES.appPublication) return parseAppPublication(raw);
  if (store === B27_STORES.messageRelease) return parseMessageRelease(raw);
  if (store === B27_STORES.inboundTruncation) return parseInboundTruncation(raw);
  failB27(B27_ERROR.CORRUPT, 'unexpected snapshot store');
}

async function readStore(ops, store) {
  const keys = (await ops.list(store)).filter((key) => typeof key === 'string').sort();
  const values = [];
  for (const key of keys) {
    const record = parseStoreRecord(store, await required(ops, store, key, `${store} record`));
    values.push(Object.freeze({ key, record }));
  }
  return Object.freeze(values);
}

async function readSnapshotFromOps(ops, groupIdHex) {
  const stores = {};
  const readSet = [];
  for (const store of SNAPSHOT_STORES) {
    const entries = await readStore(ops, store);
    stores[store] = entries;
    for (const { key, record } of entries) readSet.push([store, key, recordDigest(store, record)]);
  }
  const headEntry = stores[B27_STORES.head].find(({ key }) => key === groupIdHex);
  if (headEntry === undefined) failB27(B27_ERROR.NOT_FOUND, 'canonical head is absent');
  const head = headEntry.record;
  const groupEntries = (store) => stores[store]
    .filter(({ record }) => record.groupIdHex === undefined || record.groupIdHex === groupIdHex)
    .map(({ record }) => record);
  const transitions = groupEntries(B27_STORES.transition);
  const currentTransition = transitions.find((item) =>
    item.transitionDigestHex === head.transitionDigestHex);
  const samePath = currentTransition !== undefined
    && currentTransition.selectedPath.length === head.canonicalPath.length
    && currentTransition.selectedPath.every((item, index) =>
      item === head.canonicalPath[index]);
  if (currentTransition === undefined
    || currentTransition.groupIdHex !== head.groupIdHex
    || currentTransition.seq !== head.seq
    || currentTransition.predecessorHeadDigestHex !== head.priorHeadDigestHex
    || currentTransition.successorSnapshotDigestHex !== head.snapshotDigestHex
    || currentTransition.anchorSnapshotDigestHex !== head.anchorSnapshotDigestHex
    || currentTransition.anchorTipCommitDigestHex !== head.anchorTipCommitDigestHex
    || currentTransition.epochDec !== head.epochDec
    || currentTransition.groupContextDigestHex !== head.groupContextDigestHex
    || !samePath) {
    failB27(B27_ERROR.CORRUPT,
      'canonical head lacks a coherent transition audit record');
  }
  return Object.freeze({
    head,
    retained: Object.freeze(groupEntries(B27_STORES.retained)),
    released: Object.freeze(groupEntries(B27_STORES.released)),
    inputs: Object.freeze(groupEntries(B27_STORES.input)),
    edges: Object.freeze(groupEntries(B27_STORES.edge)),
    passes: Object.freeze(groupEntries(B27_STORES.pass)),
    transitions: Object.freeze(transitions),
    invalidations: Object.freeze(groupEntries(B27_STORES.invalidation)),
    generations: Object.freeze(groupEntries(B27_STORES.generation)),
    generationTruncation: groupEntries(B27_STORES.generationTruncation)[0] ?? null,
    activeLocal: groupEntries(B27_STORES.activeLocal)[0] ?? null,
    publications: Object.freeze(groupEntries(B27_STORES.publication)),
    readSetDigestHex: digestHex(canonicalB27Bytes('JOURNAL-READ-SET', readSet)),
  });
}

function rebuildInput(record, changes) {
  const fields = {};
  for (const field of INPUT_FIELDS) {
    if (!['format', 'version', 'inputDigestHex', 'commitDigestHex',
      'commitByteLength'].includes(field)) fields[field] = record[field];
  }
  return buildInput({ ...fields, ...changes });
}

function rebuildGeneration(record, changes) {
  const fields = {};
  for (const field of GENERATION_FIELDS) {
    if (!['format', 'version', 'generationDigestHex'].includes(field)) fields[field] = record[field];
  }
  return buildGeneration({ ...fields, ...changes });
}

function rebuildMessageState(record, changes) {
  const fields = {};
  for (const field of MESSAGE_STATE_FIELDS) {
    if (!['format', 'version', 'profile', 'runtime', 'stateDigestHex'].includes(field)) {
      fields[field] = record[field];
    }
  }
  return buildMessageState({ ...fields, ...changes });
}

function rebuildOutbox(record, changes) {
  const fields = {};
  for (const field of OUTBOX_FIELDS) {
    if (!['format', 'version', 'outboxDigestHex'].includes(field)) fields[field] = record[field];
  }
  return buildOutbox({ ...fields, ...changes });
}

function rebuildInbound(record, changes) {
  const fields = {};
  for (const field of INBOUND_FIELDS) {
    if (!['format', 'version', 'inboundDigestHex'].includes(field)) {
      fields[field] = record[field];
    }
  }
  return buildInbound({ ...fields, ...changes });
}

function appPublicationKey(instanceKeyHex, ordinal, sequence) {
  assertHex64('instanceKeyHex', instanceKeyHex);
  assertSafeInteger('outbox ordinal', ordinal, 1, Number.MAX_SAFE_INTEGER);
  assertSafeInteger('publication sequence', sequence, 1,
    B27_LIMITS.maxMessagePublicationRecords);
  return `${instanceKeyHex}:${String(ordinal).padStart(16, '0')}:`+
    `${String(sequence).padStart(3, '0')}`;
}

export class B27Journal {
  #db;

  constructor(db, token) {
    if (token !== JOURNAL_TOKEN) {
      failB27(B27_ERROR.INVALID, 'B2.7 journals must be created by the namespace factory');
    }
    this.#db = db;
    Object.freeze(this);
  }

  async #initialize({ groupIdHex, accountKeyHex, signatureKeyHex, epochDec,
    groupContextDigestHex, snapshotBytes }) {
    const retained = buildRetainedState({ groupIdHex, accountKeyHex, signatureKeyHex,
      epochDec, groupContextDigestHex, snapshotBytes });
    const transition = buildTransition({
      groupIdHex, seq: 1, kind: 'INITIALIZE', predecessorHeadDigestHex: null,
      successorSnapshotDigestHex: retained.snapshotDigestHex,
      anchorSnapshotDigestHex: retained.snapshotDigestHex, selectedPath: [], displacedPath: [],
      anchorTipCommitDigestHex: null,
      passDigestHex: null, invalidationDigestHex: null, releasedStateDigests: [],
      epochDec, groupContextDigestHex,
    });
    const head = buildHead({
      groupIdHex, accountKeyHex, signatureKeyHex, seq: 1, state: B27_HEAD_STATE.STABLE,
      epochDec, groupContextDigestHex, snapshotDigestHex: retained.snapshotDigestHex,
      anchorSnapshotDigestHex: retained.snapshotDigestHex, canonicalPath: [],
      anchorTipCommitDigestHex: null,
      priorHeadDigestHex: null, transitionDigestHex: transition.transitionDigestHex,
      selectedCommitDigestHex: null,
    });
    await this.#db.transaction(B27_STORE_NAMES, async (ops) => {
      for (const store of B27_STORE_NAMES) {
        if ((await ops.list(store)).length !== 0) {
          failB27(B27_ERROR.CAS_CONFLICT, 'private one-group B2.7 journal is not empty');
        }
      }
      await Promise.all([
        ops.put(B27_STORES.retained, retained.snapshotDigestHex, retained),
        ops.put(B27_STORES.transition, transition.transitionDigestHex, transition),
        ops.put(B27_STORES.head, groupIdHex, head),
        ops.put(B27_STORES.activeLocal, groupIdHex,
          buildActiveLocal({ groupIdHex, generationDigestHex: null })),
      ]);
    });
    return head;
  }

  async snapshot(groupIdHex) {
    assertGroupIdHex(groupIdHex);
    return this.#db.transaction(SNAPSHOT_STORES,
      async (ops) => readSnapshotFromOps(ops, groupIdHex));
  }

  async readHead(groupIdHex) {
    const snapshot = await this.snapshot(groupIdHex);
    const retained = snapshot.retained.find((item) =>
      item.snapshotDigestHex === snapshot.head.snapshotDigestHex);
    if (retained === undefined && snapshot.head.state !== B27_HEAD_STATE.UNRECOVERABLE) {
      const released = snapshot.released.find((item) =>
        item.snapshotDigestHex === snapshot.head.snapshotDigestHex);
      if (released !== undefined) {
        failB27(B27_ERROR.RELEASED, 'canonical state was logically released', { released });
      }
      failB27(B27_ERROR.CORRUPT, 'canonical retained state is missing');
    }
    return Object.freeze({ head: snapshot.head, retained: retained ?? null,
      transition: snapshot.transitions.find((item) =>
        item.transitionDigestHex === snapshot.head.transitionDigestHex) ?? null });
  }

  async readRetained(snapshotDigestHex) {
    assertHex64('snapshotDigestHex', snapshotDigestHex);
    return this.#db.transaction([B27_STORES.retained, B27_STORES.released], async (ops) => {
      const raw = await ops.get(B27_STORES.retained, snapshotDigestHex);
      if (raw !== undefined) return parseRetainedState(raw);
      const released = await ops.get(B27_STORES.released, snapshotDigestHex);
      if (released !== undefined) {
        const tombstone = parseRelease(released);
        failB27(B27_ERROR.RELEASED, 'retained state was logically released', { tombstone });
      }
      failB27(B27_ERROR.NOT_FOUND, 'retained state is absent', { snapshotDigestHex });
    });
  }

  async admitCommit(groupIdHex, commitBytes) {
    assertGroupIdHex(groupIdHex);
    assertBytes('Commit bytes', commitBytes, { min: 1, max: B27_LIMITS.maxCommitBytes });
    const proposed = buildInput({ groupIdHex, commitBytes });
    return this.#db.transaction(SNAPSHOT_STORES, async (ops) => {
      const view = await readSnapshotFromOps(ops, groupIdHex);
      if (view.head.state !== B27_HEAD_STATE.STABLE) {
        failB27(B27_ERROR.UNRECOVERABLE, 'input admission requires a stable group');
      }
      const generation = view.generations.find((item) =>
        item.commitDigestHex === proposed.commitDigestHex);
      if (generation !== undefined) {
        if (!bytesEqual(generation.commitBytes, proposed.commitBytes)) {
          failB27(B27_ERROR.CORRUPT, 'local generation digest collision');
        }
        return Object.freeze({ status: 'own_echo', generation });
      }
      const existing = view.inputs.find((item) =>
        item.commitDigestHex === proposed.commitDigestHex);
      if (existing !== undefined) {
        if (!bytesEqual(existing.commitBytes, proposed.commitBytes)) {
          failB27(B27_ERROR.CORRUPT, 'Commit digest collision');
        }
        return Object.freeze({ status: 'duplicate', input: existing });
      }
      if (view.inputs.length >= B27_LIMITS.maxInputs) {
        failB27(B27_ERROR.RESOURCE_LIMIT, 'retained raw-input cap is exhausted');
      }
      await ops.put(B27_STORES.input,
        inputKey(groupIdHex, proposed.commitDigestHex), proposed);
      return Object.freeze({ status: 'retained', input: proposed });
    });
  }

  async freeze(groupIdHex) {
    assertGroupIdHex(groupIdHex);
    return this.#db.transaction(SNAPSHOT_STORES, async (ops) => {
      const view = await readSnapshotFromOps(ops, groupIdHex);
      if (view.head.state !== B27_HEAD_STATE.STABLE) {
        failB27(B27_ERROR.UNRECOVERABLE, 'pass freeze requires a stable group');
      }
      if (view.passes.some((item) => item.state === B27_PASS_STATE.FROZEN)) {
        failB27(B27_ERROR.STATE_CONFLICT, 'one frozen pass already awaits settlement');
      }
      if (view.passes.length >= B27_LIMITS.maxPasses) {
        failB27(B27_ERROR.RESOURCE_LIMIT, 'pass history cap is exhausted');
      }
      const eligibleLocal = view.generations.filter((item) => [
        B27_GENERATION_STATE.ACKNOWLEDGED,
        B27_GENERATION_STATE.SELECTED,
        B27_GENERATION_STATE.LOSING,
      ].includes(item.state) && !item.contradiction);
      const closure = [...view.inputs.map((item) => item.commitDigestHex),
        ...eligibleLocal.map((item) => item.commitDigestHex)].sort();
      const unique = [...new Set(closure)];
      if (unique.length !== closure.length) failB27(B27_ERROR.CORRUPT, 'input closure collides');
      if (unique.length === 0) failB27(B27_ERROR.STATE_CONFLICT, 'cannot freeze an empty pass');
      if (unique.length > B27_LIMITS.maxInputs) {
        failB27(B27_ERROR.RESOURCE_LIMIT, 'frozen input closure exceeds its cap');
      }
      const pass = buildPass({ groupIdHex, baseHeadDigestHex: view.head.headDigestHex,
        closureCommitDigests: unique });
      if (await ops.get(B27_STORES.pass, pass.passDigestHex) !== undefined) {
        failB27(B27_ERROR.CAS_CONFLICT, 'identical frozen pass already exists');
      }
      await ops.put(B27_STORES.pass, pass.passDigestHex, pass);
      return pass;
    });
  }

  async readFrozen(passDigestHex) {
    assertHex64('passDigestHex', passDigestHex);
    return this.#db.transaction(SNAPSHOT_STORES, async (ops) => {
      const pass = parsePass(await required(ops, B27_STORES.pass, passDigestHex, 'pass'));
      if (pass.state !== B27_PASS_STATE.FROZEN) {
        failB27(B27_ERROR.STATE_CONFLICT, 'pass is already settled');
      }
      const view = await readSnapshotFromOps(ops, pass.groupIdHex);
      if (view.head.headDigestHex !== pass.baseHeadDigestHex) {
        failB27(B27_ERROR.CAS_CONFLICT, 'frozen pass no longer binds the canonical head');
      }
      const actual = [...view.inputs.map((item) => item.commitDigestHex),
        ...view.generations.filter((item) => [
          B27_GENERATION_STATE.ACKNOWLEDGED,
          B27_GENERATION_STATE.SELECTED,
          B27_GENERATION_STATE.LOSING,
        ].includes(item.state) && !item.contradiction)
          .map((item) => item.commitDigestHex)].sort();
      if (actual.length !== pass.closureCommitDigests.length
        || actual.some((item, index) => item !== pass.closureCommitDigests[index])) {
        failB27(B27_ERROR.CAS_CONFLICT, 'frozen input closure changed');
      }
      return Object.freeze({ ...view, pass });
    });
  }

  async #commitSettlement(decision) {
    const expectedHead = parseHead(decision.expectedHead);
    const settledPass = parsePass(decision.settledPass);
    const nextHead = parseHead(decision.nextHead);
    const transition = parseTransition(decision.transition);
    const invalidation = parseInvalidation(decision.invalidation);
    const states = decision.states.map(parseRetainedState);
    const edges = decision.edges.map(parseEdge);
    const inputs = decision.inputs.map(parseInput);
    const releases = decision.releases.map(parseRelease);
    const generations = decision.generations.map(parseGeneration);
    const anchorAdvanceEdge = decision.anchorAdvanceEdge === null
      ? null : parseEdge(decision.anchorAdvanceEdge);
    if (settledPass.state !== B27_PASS_STATE.SETTLED
      || nextHead.groupIdHex !== expectedHead.groupIdHex
      || nextHead.priorHeadDigestHex !== expectedHead.headDigestHex
      || nextHead.transitionDigestHex !== transition.transitionDigestHex
      || transition.predecessorHeadDigestHex !== expectedHead.headDigestHex
      || transition.anchorTipCommitDigestHex !== nextHead.anchorTipCommitDigestHex
      || transition.passDigestHex !== settledPass.passDigestHex
      || transition.invalidationDigestHex !== invalidation.invalidationDigestHex
      || invalidation.predecessorHeadDigestHex !== expectedHead.headDigestHex
      || invalidation.successorSnapshotDigestHex !== nextHead.snapshotDigestHex) {
      failB27(B27_ERROR.INVALID, 'settlement output has incoherent authority bindings');
    }
    return this.#db.transaction(SETTLEMENT_TRANSACTION_STORES, async (ops) => {
      const current = await readSnapshotFromOps(ops, expectedHead.groupIdHex);
      if (current.head.headDigestHex !== expectedHead.headDigestHex
        || current.readSetDigestHex !== decision.expectedReadSetDigestHex) {
        failB27(B27_ERROR.CAS_CONFLICT, 'complete settlement read set changed');
      }
      const currentPass = current.passes.find((item) =>
        item.passDigestHex === settledPass.passDigestHex);
      if (currentPass?.state !== B27_PASS_STATE.FROZEN) {
        failB27(B27_ERROR.CAS_CONFLICT, 'frozen pass disappeared or changed');
      }
      const anchorChanged = nextHead.anchorSnapshotDigestHex
        !== expectedHead.anchorSnapshotDigestHex;
      if (!anchorChanged) {
        if (anchorAdvanceEdge !== null
          || nextHead.anchorTipCommitDigestHex !== expectedHead.anchorTipCommitDigestHex) {
          failB27(B27_ERROR.INVALID,
            'unchanged anchor cannot change its producing Commit binding');
        }
      } else {
        const releasedPriorAnchor = releases.some((release) =>
          release.snapshotDigestHex === expectedHead.anchorSnapshotDigestHex);
        const successorRetained = states.find((state) =>
          state.snapshotDigestHex === nextHead.anchorSnapshotDigestHex);
        const replayAuthorizedAdvance = current.edges.some((edge) =>
          edge.edgeDigestHex === anchorAdvanceEdge?.edgeDigestHex)
          || inputs.some((input) => input.commitDigestHex
            === anchorAdvanceEdge?.commitDigestHex
            && (input.state === B27_INPUT_STATE.STALE
              || (input.state === B27_INPUT_STATE.EDGE
                && input.edgeDigestHex === anchorAdvanceEdge?.edgeDigestHex)))
          || generations.some((generation) =>
            generationAuthorityDigest(generation)
              === anchorAdvanceEdge?.localGenerationDigestHex
            && generation.commitDigestHex === anchorAdvanceEdge?.commitDigestHex);
        if (anchorAdvanceEdge === null
          || nextHead.anchorTipCommitDigestHex === null
          || !replayAuthorizedAdvance
          || anchorAdvanceEdge.groupIdHex !== expectedHead.groupIdHex
          || anchorAdvanceEdge.parentSnapshotDigestHex
            !== expectedHead.anchorSnapshotDigestHex
          || anchorAdvanceEdge.successorSnapshotDigestHex
            !== nextHead.anchorSnapshotDigestHex
          || anchorAdvanceEdge.commitDigestHex !== nextHead.anchorTipCommitDigestHex
          || successorRetained === undefined
          || !releasedPriorAnchor) {
          failB27(B27_ERROR.INVALID,
            'advanced anchor lacks its exact replay-authorized producing Commit binding');
        }
      }
      const writes = [];
      const releasedSnapshots = new Set(releases.map((release) => release.snapshotDigestHex));
      const canonicalSnapshots = new Set([nextHead.anchorSnapshotDigestHex]);
      for (const commitDigestHex of nextHead.canonicalPath) {
        const edge = edges.find((candidate) =>
          candidate.commitDigestHex === commitDigestHex);
        if (edge === undefined) {
          failB27(B27_ERROR.CORRUPT,
            'canonical settlement path lacks its retained replay edge');
        }
        canonicalSnapshots.add(edge.successorSnapshotDigestHex);
      }
      const messageStateEntries = [];
      for (const key of await ops.list(B27_STORES.messageState)) {
        if (typeof key !== 'string') continue;
        const state = parseMessageState(await required(
          ops, B27_STORES.messageState, key, 'message-state record'));
        if (state.groupIdHex === expectedHead.groupIdHex) {
          if (key !== messageStateKey(state.groupIdHex, state.instanceKeyHex)) {
            failB27(B27_ERROR.CORRUPT,
              'message-state record is stored under a contradictory key');
          }
          messageStateEntries.push({ key, state });
        }
      }
      const deferredInboundBases = new Set();
      for (const key of await ops.list(B27_STORES.appInbound)) {
        if (typeof key !== 'string') continue;
        const inbound = parseInbound(await required(
          ops, B27_STORES.appInbound, key, 'inbound record'));
        if (inbound.groupIdHex === expectedHead.groupIdHex
          && inbound.disposition === B27_INBOUND_STATE.DEFERRED) {
          deferredInboundBases.add(inbound.baseRetainedSnapshotDigestHex);
        }
      }
      const releasePlans = [];
      for (const release of releases) {
        const retained = current.retained.find((state) =>
          state.snapshotDigestHex === release.snapshotDigestHex);
        if (retained === undefined) {
          failB27(B27_ERROR.CAS_CONFLICT,
            'released message instance lost its retained state');
        }
        const storedInstanceKeys = new Set();
        for (const { state } of messageStateEntries) {
          if (state.baseRetainedSnapshotDigestHex !== retained.snapshotDigestHex
            || storedInstanceKeys.has(state.instanceKeyHex)) continue;
          storedInstanceKeys.add(state.instanceKeyHex);
        }
        const deferredOnly = storedInstanceKeys.size === 0
          && deferredInboundBases.has(retained.snapshotDigestHex);
        if (!deferredOnly) {
          const tipCommitDigestHex = resolveTipCommitDigestHex({
            snapshotDigestHex: retained.snapshotDigestHex,
            head: current.head,
            edges: current.edges,
            generations: current.generations,
          });
          const expectedInstanceKeyHex = messageInstanceKey({
            groupIdHex: retained.groupIdHex,
            tipCommitDigestHex,
            epochDec: retained.epochDec,
            groupContextDigestHex: retained.groupContextDigestHex,
            localMemberIdentityHex: current.head.accountKeyHex,
          });
          for (const instanceKeyHex of storedInstanceKeys) {
            if (instanceKeyHex !== expectedInstanceKeyHex) {
              failB27(B27_ERROR.CORRUPT,
                'stored message-state identity contradicts pre-settlement transcript evidence');
            }
          }
        }
        releasePlans.push({ retained, storedInstanceKeys });
      }
      const releasedInstanceKeys = new Set();
      for (const { retained, storedInstanceKeys } of releasePlans) {
        for (const instanceKeyHex of storedInstanceKeys) {
          if (releasedInstanceKeys.has(instanceKeyHex)) continue;
          writes.push(...await releaseMessageInstanceRecords(ops, {
            groupIdHex: retained.groupIdHex,
            instanceKeyHex,
            releaseAuthorityDigestHex: transition.transitionDigestHex,
          }));
          releasedInstanceKeys.add(instanceKeyHex);
        }
        writes.push(...await invalidateDeferredInboundForBase(ops, {
          groupIdHex: retained.groupIdHex,
          baseRetainedSnapshotDigestHex: retained.snapshotDigestHex,
        }));
      }
      for (const { key, state } of messageStateEntries) {
        const mustRelease = releasedSnapshots.has(state.baseRetainedSnapshotDigestHex);
        if (mustRelease) continue;
        if (canonicalSnapshots.has(state.baseRetainedSnapshotDigestHex)
          && state.state === B27_MESSAGE_STATE.SUSPENDED) {
          writes.push(ops.put(B27_STORES.messageState, key,
            rebuildMessageState(state, { state: B27_MESSAGE_STATE.ACTIVE })));
        } else if (!canonicalSnapshots.has(state.baseRetainedSnapshotDigestHex)
          && state.state === B27_MESSAGE_STATE.ACTIVE) {
          writes.push(ops.put(B27_STORES.messageState, key,
            rebuildMessageState(state, { state: B27_MESSAGE_STATE.SUSPENDED })));
        }
        if (state.state !== (canonicalSnapshots
          .has(state.baseRetainedSnapshotDigestHex)
          ? B27_MESSAGE_STATE.ACTIVE : B27_MESSAGE_STATE.SUSPENDED)) {
          for (const outboxKeyValue of await ops.list(B27_STORES.appOutbox)) {
            if (typeof outboxKeyValue !== 'string'
              || !outboxKeyValue.startsWith(state.instanceKeyHex + ':')) continue;
            const outbox = parseOutbox(await required(
              ops, B27_STORES.appOutbox, outboxKeyValue, 'outbox record'));
            if ([B27_OUTBOX_STATE.ACKNOWLEDGED, B27_OUTBOX_STATE.FAILED_DISCARDED,
              B27_OUTBOX_STATE.INVALIDATED].includes(outbox.state)) continue;
            let outboxState = outbox.state;
            if (!canonicalSnapshots.has(state.baseRetainedSnapshotDigestHex)) {
              outboxState = B27_OUTBOX_STATE.SUSPENDED;
            } else if (outbox.state === B27_OUTBOX_STATE.SUSPENDED) {
              outboxState = outbox.attemptCount === 0
                ? B27_OUTBOX_STATE.DURABLE : B27_OUTBOX_STATE.ATTEMPTED;
            }
            if (outboxState !== outbox.state) {
              writes.push(ops.put(B27_STORES.appOutbox, outboxKeyValue,
                rebuildOutbox(outbox, { state: outboxState })));
            }
          }
        }
      }
      for (const state of states) {
        const prior = await ops.get(B27_STORES.retained, state.snapshotDigestHex);
        if (prior !== undefined
          && parseRetainedState(prior).retainedDigestHex !== state.retainedDigestHex) {
          failB27(B27_ERROR.CORRUPT, 'retained-state content-address collision');
        }
        writes.push(ops.put(B27_STORES.retained, state.snapshotDigestHex, state));
      }
      for (const edge of edges) {
        const key = edgeKey(edge.groupIdHex, edge.parentSnapshotDigestHex, edge.commitDigestHex);
        const prior = await ops.get(B27_STORES.edge, key);
        if (prior !== undefined && parseEdge(prior).edgeDigestHex !== edge.edgeDigestHex) {
          failB27(B27_ERROR.CORRUPT, 'replay-edge identity collision');
        }
        writes.push(ops.put(B27_STORES.edge, key, edge));
      }
      const retainedEdgeKeys = new Set(edges.map((edge) =>
        edgeKey(edge.groupIdHex, edge.parentSnapshotDigestHex, edge.commitDigestHex)));
      for (const priorEdge of current.edges) {
        const key = edgeKey(priorEdge.groupIdHex,
          priorEdge.parentSnapshotDigestHex, priorEdge.commitDigestHex);
        if (!retainedEdgeKeys.has(key)) writes.push(ops.delete(B27_STORES.edge, key));
      }
      for (const input of inputs) writes.push(ops.put(B27_STORES.input,
        inputKey(input.groupIdHex, input.commitDigestHex), input));
      for (const generation of generations) writes.push(ops.put(B27_STORES.generation,
        generationKey(generation.groupIdHex, generation.commitDigestHex), generation));
      const activePointer = current.activeLocal;
      if (activePointer?.generationDigestHex !== null) {
        const priorActive = current.generations.find((item) =>
          item.generationDigestHex === activePointer.generationDigestHex);
        const nextActive = generations.find((item) =>
          item.commitDigestHex === priorActive?.commitDigestHex);
        if (nextActive !== undefined && [B27_GENERATION_STATE.SELECTED,
          B27_GENERATION_STATE.LOSING, B27_GENERATION_STATE.REJECTED]
          .includes(nextActive.state)) {
          writes.push(ops.put(B27_STORES.activeLocal, nextActive.groupIdHex,
            buildActiveLocal({ groupIdHex: nextActive.groupIdHex,
              generationDigestHex: null })));
        }
      }
      writes.push(ops.put(B27_STORES.pass, settledPass.passDigestHex, settledPass));
      writes.push(ops.put(B27_STORES.invalidation,
        invalidation.invalidationDigestHex, invalidation));
      writes.push(ops.put(B27_STORES.transition, transition.transitionDigestHex, transition));
      for (const release of releases) {
        const retainedRaw = await ops.get(B27_STORES.retained, release.snapshotDigestHex);
        if (retainedRaw === undefined
          || parseRetainedState(retainedRaw).retainedDigestHex !== release.retainedDigestHex) {
          failB27(B27_ERROR.CAS_CONFLICT, 'logical release target changed');
        }
        writes.push(ops.put(B27_STORES.released, release.snapshotDigestHex, release));
        writes.push(ops.delete(B27_STORES.retained, release.snapshotDigestHex));
      }
      writes.push(ops.put(B27_STORES.head, nextHead.groupIdHex, nextHead));
      await Promise.all(writes);
      return Object.freeze({ head: nextHead, pass: settledPass, transition, invalidation,
        states: Object.freeze(states), edges: Object.freeze(edges),
        inputs: Object.freeze(inputs), releases: Object.freeze(releases) });
    });
  }

  async #markUnrecoverable(expectedHead) {
    const prior = parseHead(expectedHead);
    if (prior.state !== B27_HEAD_STATE.STABLE) {
      failB27(B27_ERROR.UNRECOVERABLE, 'group is already unrecoverable');
    }
    const transition = buildTransition({
      groupIdHex: prior.groupIdHex,
      seq: prior.seq + 1,
      kind: 'MARK_UNRECOVERABLE',
      predecessorHeadDigestHex: prior.headDigestHex,
      successorSnapshotDigestHex: prior.snapshotDigestHex,
      anchorSnapshotDigestHex: prior.anchorSnapshotDigestHex,
      anchorTipCommitDigestHex: prior.anchorTipCommitDigestHex,
      selectedPath: prior.canonicalPath,
      displacedPath: [],
      passDigestHex: null,
      invalidationDigestHex: null,
      releasedStateDigests: [],
      epochDec: prior.epochDec,
      groupContextDigestHex: prior.groupContextDigestHex,
    });
    const next = buildHead({
      groupIdHex: prior.groupIdHex,
      accountKeyHex: prior.accountKeyHex,
      signatureKeyHex: prior.signatureKeyHex,
      seq: prior.seq + 1,
      state: B27_HEAD_STATE.UNRECOVERABLE,
      epochDec: prior.epochDec,
      groupContextDigestHex: prior.groupContextDigestHex,
      snapshotDigestHex: prior.snapshotDigestHex,
      anchorSnapshotDigestHex: prior.anchorSnapshotDigestHex,
      anchorTipCommitDigestHex: prior.anchorTipCommitDigestHex,
      canonicalPath: prior.canonicalPath,
      priorHeadDigestHex: prior.headDigestHex,
      transitionDigestHex: transition.transitionDigestHex,
      selectedCommitDigestHex: prior.selectedCommitDigestHex,
    });
    return this.#db.transaction(SNAPSHOT_STORES, async (ops) => {
      const current = await readSnapshotFromOps(ops, prior.groupIdHex);
      if (current.head.headDigestHex !== prior.headDigestHex) {
        failB27(B27_ERROR.CAS_CONFLICT,
          'canonical head changed before unrecoverable transition');
      }
      await Promise.all([
        ops.put(B27_STORES.transition, transition.transitionDigestHex, transition),
        ops.put(B27_STORES.head, next.groupIdHex, next),
      ]);
      return Object.freeze({ head: next, transition });
    });
  }

  async #commitPrepared({ expectedHead, generation, pendingRetained }) {
    const head = parseHead(expectedHead);
    const next = parseGeneration(generation);
    const retained = parseRetainedState(pendingRetained);
    if (next.parentHeadDigestHex !== head.headDigestHex
      || next.parentSnapshotDigestHex !== head.snapshotDigestHex
      || next.pendingSnapshotDigestHex !== retained.snapshotDigestHex) {
      failB27(B27_ERROR.INVALID, 'local generation does not bind its parent');
    }
    return this.#db.transaction(SETTLEMENT_TRANSACTION_STORES, async (ops) => {
      const view = await readSnapshotFromOps(ops, head.groupIdHex);
      if (view.head.headDigestHex !== head.headDigestHex
        || view.activeLocal?.generationDigestHex !== null) {
        failB27(B27_ERROR.CAS_CONFLICT, 'head or active-local pointer changed');
      }
      if (view.passes.some((item) => item.state === B27_PASS_STATE.FROZEN)) {
        failB27(B27_ERROR.STATE_CONFLICT,
          'local preparation is excluded while a settlement pass is frozen');
      }
      const generationFloor = Math.max(view.generationTruncation?.throughGeneration ?? 0,
        ...view.generations.map((item) => item.generation));
      if (next.generation !== generationFloor + 1) {
        failB27(B27_ERROR.CAS_CONFLICT, 'local generation ordinal changed');
      }
      const writes = [
        ops.put(B27_STORES.retained, retained.snapshotDigestHex, retained),
        ops.put(B27_STORES.generation,
          generationKey(next.groupIdHex, next.commitDigestHex), next),
        ops.put(B27_STORES.activeLocal, next.groupIdHex,
          buildActiveLocal({ groupIdHex: next.groupIdHex,
            generationDigestHex: next.generationDigestHex })),
      ];
      if (view.generations.length >= B27_LIMITS.maxGenerations) {
        const evictableStates = new Set([
          B27_GENERATION_STATE.CANCELLED,
          B27_GENERATION_STATE.DISCARDED,
          B27_GENERATION_STATE.REJECTED,
        ]);
        const evicted = [...view.generations]
          .sort((left, right) => left.generation - right.generation)
          .find((candidate) => evictableStates.has(candidate.state)
            && !candidate.contradiction
            && !view.edges.some((edge) => edge.localGenerationDigestHex
              === generationAuthorityDigest(candidate)));
        if (evicted === undefined) {
          failB27(B27_ERROR.RESOURCE_LIMIT,
            'local generation history cap has no safely evictable terminal record');
        }
        const pendingRequired = view.head.snapshotDigestHex === evicted.pendingSnapshotDigestHex
          || view.head.anchorSnapshotDigestHex === evicted.pendingSnapshotDigestHex
          || view.edges.some((edge) => edge.parentSnapshotDigestHex
            === evicted.pendingSnapshotDigestHex
            || edge.successorSnapshotDigestHex === evicted.pendingSnapshotDigestHex)
          || view.generations.some((candidate) => candidate.commitDigestHex
            !== evicted.commitDigestHex
            && candidate.pendingSnapshotDigestHex === evicted.pendingSnapshotDigestHex);
        const marker = buildGenerationTruncation({
          groupIdHex: evicted.groupIdHex,
          throughGeneration: evicted.generation,
          evictedCommitDigestHex: evicted.commitDigestHex,
          evictedGenerationAuthorityDigestHex: generationAuthorityDigest(evicted),
          evictedGenerationDigestHex: evicted.generationDigestHex,
          priorMarkerDigestHex: view.generationTruncation?.markerDigestHex ?? null,
        });
        writes.push(
          ops.delete(B27_STORES.generation,
            generationKey(evicted.groupIdHex, evicted.commitDigestHex)),
          ops.put(B27_STORES.generationTruncation, evicted.groupIdHex, marker),
        );
        const publicationPrefix = `${evicted.groupIdHex}:${evicted.commitDigestHex}:`;
        for (const key of await ops.list(B27_STORES.publication)) {
          if (typeof key === 'string' && key.startsWith(publicationPrefix)) {
            writes.push(ops.delete(B27_STORES.publication, key));
          }
        }
        if (!pendingRequired) {
          const retainedRaw = await ops.get(
            B27_STORES.retained, evicted.pendingSnapshotDigestHex);
          if (retainedRaw !== undefined) {
            const evictedRetained = parseRetainedState(retainedRaw);
            const release = buildRelease({
              groupIdHex: evictedRetained.groupIdHex,
              snapshotDigestHex: evictedRetained.snapshotDigestHex,
              epochDec: evictedRetained.epochDec,
              groupContextDigestHex: evictedRetained.groupContextDigestHex,
              retainedDigestHex: evictedRetained.retainedDigestHex,
              releaseAuthorityDigestHex: marker.markerDigestHex,
            });
            writes.push(...await releaseRetainedMessageRecords(ops, {
              head: view.head,
              retained: evictedRetained,
              tipCommitDigestHex: evicted.commitDigestHex,
              releaseAuthorityDigestHex: marker.markerDigestHex,
            }));
            writes.push(
              ops.put(B27_STORES.released, release.snapshotDigestHex, release),
              ops.delete(B27_STORES.retained, release.snapshotDigestHex),
            );
          }
        }
      }
      await Promise.all(writes);
      return next;
    });
  }

  async #replaceGeneration(expectedGeneration, nextGeneration, { clearActive = false } = {}) {
    const prior = parseGeneration(expectedGeneration);
    const next = parseGeneration(nextGeneration);
    if (prior.groupIdHex !== next.groupIdHex || prior.commitDigestHex !== next.commitDigestHex) {
      failB27(B27_ERROR.INVALID, 'generation replacement changes identity');
    }
    return this.#db.transaction(SETTLEMENT_TRANSACTION_STORES, async (ops) => {
      const raw = await required(ops, B27_STORES.generation,
        generationKey(prior.groupIdHex, prior.commitDigestHex), 'local generation');
      if (parseGeneration(raw).generationDigestHex !== prior.generationDigestHex) {
        failB27(B27_ERROR.CAS_CONFLICT, 'local generation changed');
      }
      await ops.put(B27_STORES.generation,
        generationKey(next.groupIdHex, next.commitDigestHex), next);
      if (clearActive) {
        const pointer = parseActiveLocal(await required(ops, B27_STORES.activeLocal,
          prior.groupIdHex, 'active-local pointer'));
        if (pointer.generationDigestHex !== prior.generationDigestHex) {
          failB27(B27_ERROR.CAS_CONFLICT, 'active-local pointer changed');
        }
        await ops.put(B27_STORES.activeLocal, prior.groupIdHex,
          buildActiveLocal({ groupIdHex: prior.groupIdHex, generationDigestHex: null }));
        if ([B27_GENERATION_STATE.CANCELLED, B27_GENERATION_STATE.DISCARDED,
          B27_GENERATION_STATE.REJECTED].includes(next.state)) {
          const retainedRaw = await ops.get(
            B27_STORES.retained, next.pendingSnapshotDigestHex);
          if (retainedRaw !== undefined) {
            const retained = parseRetainedState(retainedRaw);
            const currentHead = parseHead(await required(ops, B27_STORES.head,
              retained.groupIdHex, 'canonical head'));
            const release = buildRelease({
              groupIdHex: retained.groupIdHex,
              snapshotDigestHex: retained.snapshotDigestHex,
              epochDec: retained.epochDec,
              groupContextDigestHex: retained.groupContextDigestHex,
              retainedDigestHex: retained.retainedDigestHex,
              releaseAuthorityDigestHex: next.generationDigestHex,
            });
            const messageWrites = await releaseRetainedMessageRecords(ops, {
              head: currentHead,
              retained,
              tipCommitDigestHex: next.commitDigestHex,
              releaseAuthorityDigestHex: next.generationDigestHex,
            });
            await Promise.all([
              ...messageWrites,
              ops.put(B27_STORES.released, release.snapshotDigestHex, release),
              ops.delete(B27_STORES.retained, release.snapshotDigestHex),
            ]);
          }
        }
      } else {
        const pointer = parseActiveLocal(await required(ops, B27_STORES.activeLocal,
          prior.groupIdHex, 'active-local pointer'));
        if (pointer.generationDigestHex === prior.generationDigestHex) {
          await ops.put(B27_STORES.activeLocal, prior.groupIdHex,
            buildActiveLocal({ groupIdHex: prior.groupIdHex,
              generationDigestHex: next.generationDigestHex }));
        }
      }
      return next;
    });
  }

  async #appendPublication({ expectedGeneration, evidence, nextGeneration }) {
    const prior = parseGeneration(expectedGeneration);
    const record = parsePublication(evidence);
    const next = parseGeneration(nextGeneration);
    if (record.groupIdHex !== prior.groupIdHex
      || record.generationDigestHex !== prior.generationDigestHex
      || record.artifactDigestHex !== prior.commitDigestHex) {
      failB27(B27_ERROR.INVALID, 'publication evidence lacks exact generation authority');
    }
    return this.#db.transaction(SNAPSHOT_STORES, async (ops) => {
      const current = parseGeneration(await required(ops, B27_STORES.generation,
        generationKey(prior.groupIdHex, prior.commitDigestHex), 'local generation'));
      if (current.generationDigestHex !== prior.generationDigestHex) {
        failB27(B27_ERROR.CAS_CONFLICT, 'local generation changed before evidence append');
      }
      const prefix = `${prior.groupIdHex}:${prior.commitDigestHex}:`;
      const keys = (await ops.list(B27_STORES.publication))
        .filter((key) => typeof key === 'string' && key.startsWith(prefix)).sort();
      if (record.sequence !== keys.length + 1) {
        failB27(B27_ERROR.CAS_CONFLICT, 'publication sequence changed');
      }
      await Promise.all([
        ops.put(B27_STORES.publication,
          publicationKey(prior.groupIdHex, prior.commitDigestHex, record.sequence), record),
        ops.put(B27_STORES.generation,
          generationKey(next.groupIdHex, next.commitDigestHex), next),
      ]);
      const pointer = parseActiveLocal(await required(ops, B27_STORES.activeLocal,
        prior.groupIdHex, 'active-local pointer'));
      if (pointer.generationDigestHex === prior.generationDigestHex) {
        await ops.put(B27_STORES.activeLocal, prior.groupIdHex,
          buildActiveLocal({ groupIdHex: prior.groupIdHex,
            generationDigestHex: next.generationDigestHex }));
      }
      return Object.freeze({ evidence: record, generation: next });
    });
  }

  async reserveProbe(groupIdHex) {
    assertGroupIdHex(groupIdHex);
    return this.#db.transaction([
      B27_STORES.head, B27_STORES.retained, B27_STORES.probeReservation,
      B27_STORES.probeCompletion,
    ], async (ops) => {
      const head = parseHead(await required(ops, B27_STORES.head, groupIdHex, 'canonical head'));
      if (head.state !== B27_HEAD_STATE.STABLE) {
        failB27(B27_ERROR.UNRECOVERABLE, 'probe requires a stable canonical head');
      }
      const retained = parseRetainedState(await required(ops, B27_STORES.retained,
        head.snapshotDigestHex, 'canonical retained state'));
      const key = probeKey({ groupIdHex, tipCommitDigestHex: head.selectedCommitDigestHex,
        epochDec: head.epochDec, groupContextDigestHex: head.groupContextDigestHex,
        localMemberIdentityHex: head.accountKeyHex });
      const existing = await ops.get(B27_STORES.probeReservation, key);
      if (existing !== undefined) {
        const reservation = parseProbeReservation(existing);
        const completionRaw = await ops.get(B27_STORES.probeCompletion, key);
        const completion = completionRaw === undefined ? null : parseProbeCompletion(completionRaw);
        failB27(B27_ERROR.PROBE_ALREADY_RESERVED,
          completion === null ? 'epoch instance is reserved without completion'
            : 'epoch instance already has completed liveness evidence',
          { probeKeyHex: key, completed: completion !== null, reservation, completion });
      }
      const count = (await ops.list(B27_STORES.probeReservation)).length;
      if (count >= B27_LIMITS.maxProbeReservations) {
        failB27(B27_ERROR.RESOURCE_LIMIT, 'probe reservation ledger is full');
      }
      const reservation = buildProbeReservation({
        groupIdHex, headDigestHex: head.headDigestHex,
        tipCommitDigestHex: head.selectedCommitDigestHex, epochDec: head.epochDec,
        groupContextDigestHex: head.groupContextDigestHex,
        localMemberIdentityHex: head.accountKeyHex, probeKeyHex: key,
      });
      await ops.put(B27_STORES.probeReservation, key, reservation);
      return Object.freeze({ reservation, retained });
    });
  }

  async completeProbe({ probeKeyHex, ciphertextDigestHex, plaintextDigestHex,
    peerIdentityHex }) {
    assertHex64('probeKeyHex', probeKeyHex);
    return this.#db.transaction([
      B27_STORES.probeReservation, B27_STORES.probeCompletion,
    ], async (ops) => {
      const reservation = parseProbeReservation(await required(ops,
        B27_STORES.probeReservation, probeKeyHex, 'probe reservation'));
      const completion = buildProbeCompletion({ probeKeyHex,
        reservationDigestHex: reservation.reservationDigestHex, ciphertextDigestHex,
        plaintextDigestHex, peerIdentityHex });
      const existing = await ops.get(B27_STORES.probeCompletion, probeKeyHex);
      if (existing !== undefined) {
        const prior = parseProbeCompletion(existing);
        if (prior.completionDigestHex !== completion.completionDigestHex) {
          failB27(B27_ERROR.CORRUPT, 'probe completion cannot be replaced');
        }
        return Object.freeze({ status: 'duplicate', reservation, completion: prior });
      }
      await ops.put(B27_STORES.probeCompletion, probeKeyHex, completion);
      return Object.freeze({ status: 'completed', reservation, completion });
    });
  }

  async readProbe(probeKeyHex) {
    assertHex64('probeKeyHex', probeKeyHex);
    return this.#db.transaction([
      B27_STORES.probeReservation, B27_STORES.probeCompletion,
    ], async (ops) => {
      const raw = await ops.get(B27_STORES.probeReservation, probeKeyHex);
      if (raw === undefined) return null;
      const completionRaw = await ops.get(B27_STORES.probeCompletion, probeKeyHex);
      return Object.freeze({ reservation: parseProbeReservation(raw),
        completion: completionRaw === undefined ? null : parseProbeCompletion(completionRaw) });
    });
  }

  async readMessageContext(groupIdHex) {
    assertGroupIdHex(groupIdHex);
    return this.#db.transaction(MESSAGE_TRANSACTION_STORES, async (ops) => {
      const head = parseHead(await required(ops, B27_STORES.head, groupIdHex,
        'canonical head'));
      if (head.state !== B27_HEAD_STATE.STABLE) {
        failB27(B27_ERROR.UNRECOVERABLE, 'application messages require a stable head');
      }
      const retained = parseRetainedState(await required(ops, B27_STORES.retained,
        head.snapshotDigestHex, 'canonical retained state'));
      const tipAuthority = await readTipAuthority(
        ops, groupIdHex, retained.snapshotDigestHex, head);
      const { tipCommitDigestHex, producingVerifiedLeafDigestHex } = tipAuthority;
      const instanceKeyHex = messageInstanceKey({ groupIdHex,
        tipCommitDigestHex, epochDec: head.epochDec,
        groupContextDigestHex: head.groupContextDigestHex,
        localMemberIdentityHex: head.accountKeyHex });
      const key = messageStateKey(groupIdHex, instanceKeyHex);
      const releaseRaw = await ops.get(B27_STORES.messageRelease, key);
      if (releaseRaw !== undefined) {
        failB27(B27_ERROR.RELEASED, 'message instance was logically released',
          { release: parseMessageRelease(releaseRaw) });
      }
      const stateRaw = await ops.get(B27_STORES.messageState, key);
      if (stateRaw === undefined) {
        return Object.freeze({ head, retained, instanceKeyHex,
          tipCommitDigestHex, producingVerifiedLeafDigestHex,
          state: null, snapshot: null, canonical: true });
      }
      const state = parseMessageState(stateRaw);
      if (state.instanceKeyHex !== instanceKeyHex
        || state.baseRetainedSnapshotDigestHex !== retained.snapshotDigestHex
        || state.groupContextDigestHex !== head.groupContextDigestHex
        || state.epochDec !== head.epochDec) {
        failB27(B27_ERROR.CORRUPT, 'message-state does not bind the canonical instance');
      }
      const snapshot = parseMessageSnapshot(await required(ops,
        B27_STORES.messageSnapshot,
        messageSnapshotKey(instanceKeyHex, state.snapshotDigestHex),
        'current message snapshot'));
      if (snapshot.instanceKeyHex !== instanceKeyHex
        || snapshot.snapshotDigestHex !== state.snapshotDigestHex) {
        failB27(B27_ERROR.CORRUPT, 'message snapshot lacks current-state authority');
      }
      return Object.freeze({ head, retained, instanceKeyHex,
        tipCommitDigestHex, producingVerifiedLeafDigestHex,
        state, snapshot, canonical: true });
    });
  }

  async readRetainedMessageContext(groupIdHex, retainedSnapshotDigestHex) {
    assertGroupIdHex(groupIdHex);
    assertHex64('retainedSnapshotDigestHex', retainedSnapshotDigestHex);
    return this.#db.transaction(SETTLEMENT_TRANSACTION_STORES, async (ops) => {
      const head = parseHead(await required(
        ops, B27_STORES.head, groupIdHex, 'canonical head'));
      if (head.state !== B27_HEAD_STATE.STABLE) {
        failB27(B27_ERROR.UNRECOVERABLE,
          'historical application messages require a stable head');
      }
      const retainedRaw = await ops.get(B27_STORES.retained, retainedSnapshotDigestHex);
      if (retainedRaw === undefined) {
        const releaseRaw = await ops.get(B27_STORES.released, retainedSnapshotDigestHex);
        if (releaseRaw !== undefined) {
          failB27(B27_ERROR.RELEASED, 'historical retained state was released',
            { release: parseRelease(releaseRaw) });
        }
        failB27(B27_ERROR.CORRUPT, 'historical retained state is missing');
      }
      const retained = parseRetainedState(retainedRaw);
      if (retained.groupIdHex !== groupIdHex) {
        failB27(B27_ERROR.INVALID, 'historical state belongs to another group');
      }
      const edges = [];
      for (const key of await ops.list(B27_STORES.edge)) {
        if (typeof key !== 'string') continue;
        const edge = parseEdge(await required(ops, B27_STORES.edge, key, 'replay edge'));
        if (edge.groupIdHex === groupIdHex) edges.push(edge);
      }
      const canonicalSnapshots = new Set([head.anchorSnapshotDigestHex]);
      for (const commitDigestHex of head.canonicalPath) {
        const edge = edges.find((candidate) =>
          candidate.commitDigestHex === commitDigestHex);
        if (edge === undefined) failB27(B27_ERROR.CORRUPT,
          'canonical path lacks its retained replay edge');
        canonicalSnapshots.add(edge.successorSnapshotDigestHex);
      }
      const generations = await groupRecords(
        ops, B27_STORES.generation, parseGeneration, groupIdHex,
        'local generation');
      const tipCommitDigestHex = resolveTipCommitDigestHex({
        snapshotDigestHex: retainedSnapshotDigestHex,
        head,
        edges,
        generations,
      });
      const producingVerifiedLeafDigestHexValue = producingVerifiedLeafDigestHex(
        retainedSnapshotDigestHex, tipCommitDigestHex, edges);
      const instanceKeyHex = messageInstanceKey({
        groupIdHex,
        tipCommitDigestHex,
        epochDec: retained.epochDec,
        groupContextDigestHex: retained.groupContextDigestHex,
        localMemberIdentityHex: head.accountKeyHex,
      });
      const key = messageStateKey(groupIdHex, instanceKeyHex);
      const releaseRaw = await ops.get(B27_STORES.messageRelease, key);
      if (releaseRaw !== undefined) {
        failB27(B27_ERROR.RELEASED, 'historical message instance was released',
          { release: parseMessageRelease(releaseRaw) });
      }
      const stateRaw = await ops.get(B27_STORES.messageState, key);
      const state = stateRaw === undefined ? null : parseMessageState(stateRaw);
      if (state !== null
        && (state.instanceKeyHex !== instanceKeyHex
          || state.baseRetainedSnapshotDigestHex !== retained.snapshotDigestHex
          || state.groupContextDigestHex !== retained.groupContextDigestHex
          || state.epochDec !== retained.epochDec)) {
        failB27(B27_ERROR.CORRUPT,
          'message-state does not bind the historical retained instance');
      }
      const snapshot = state === null ? null : parseMessageSnapshot(await required(
        ops, B27_STORES.messageSnapshot,
        messageSnapshotKey(instanceKeyHex, state.snapshotDigestHex),
        'historical message snapshot'));
      if (snapshot !== null
        && (snapshot.instanceKeyHex !== instanceKeyHex
          || snapshot.snapshotDigestHex !== state.snapshotDigestHex)) {
        failB27(B27_ERROR.CORRUPT,
          'historical message snapshot lacks current-state authority');
      }
      return Object.freeze({ head, retained, instanceKeyHex, tipCommitDigestHex,
        producingVerifiedLeafDigestHex: producingVerifiedLeafDigestHexValue,
        state, snapshot,
        canonical: canonicalSnapshots.has(retainedSnapshotDigestHex) });
    });
  }

  async findOutboxRequest(instanceKeyHex, requestId) {
    assertHex64('instanceKeyHex', instanceKeyHex);
    assertString('requestId', requestId,
      { min: 1, max: B27_LIMITS.maxRequestIdBytes, pattern: /^[\x21-\x7e]+$/ });
    return this.#db.transaction([B27_STORES.appOutbox], async (ops) => {
      for (const key of await ops.list(B27_STORES.appOutbox)) {
        if (typeof key !== 'string' || !key.startsWith(`${instanceKeyHex}:`)) continue;
        const record = parseOutbox(await required(ops, B27_STORES.appOutbox, key,
          'outbox record'));
        if (record.requestId === requestId) return record;
      }
      return null;
    });
  }

  async readOutbox(instanceKeyHex, ordinal) {
    const raw = await this.#db.transaction([B27_STORES.appOutbox], (ops) =>
      ops.get(B27_STORES.appOutbox, outboxKey(instanceKeyHex, ordinal)));
    if (raw === undefined) failB27(B27_ERROR.NOT_FOUND, 'outbox record is absent');
    return parseOutbox(raw);
  }

  async readInbound(instanceKeyHex, ciphertextDigestHex) {
    const raw = await this.#db.transaction([B27_STORES.appInbound], (ops) =>
      ops.get(B27_STORES.appInbound, inboundKey(instanceKeyHex, ciphertextDigestHex)));
    return raw === undefined ? null : parseInbound(raw);
  }

  async readAppPublications(instanceKeyHex, ordinal) {
    assertHex64('instanceKeyHex', instanceKeyHex);
    assertSafeInteger('outbox ordinal', ordinal, 1, Number.MAX_SAFE_INTEGER);
    const prefix = instanceKeyHex + ':' + String(ordinal).padStart(16, '0') + ':';
    return this.#db.transaction([B27_STORES.appPublication], async (ops) => {
      const records = [];
      for (const key of (await ops.list(B27_STORES.appPublication)).sort()) {
        if (typeof key !== 'string' || !key.startsWith(prefix)) continue;
        records.push(parseAppPublication(await required(
          ops, B27_STORES.appPublication, key, 'application publication record')));
      }
      return Object.freeze(records);
    });
  }

  async #commitOutbound({ expectedHead, expectedRetained, expectedMessageState,
    nextMessageState, nextSnapshot, outbox }) {
    const head = parseHead(expectedHead);
    const retained = parseRetainedState(expectedRetained);
    const prior = expectedMessageState === null ? null : parseMessageState(expectedMessageState);
    const next = parseMessageState(nextMessageState);
    const snapshot = parseMessageSnapshot(nextSnapshot);
    const queued = parseOutbox(outbox);
    if (next.groupIdHex !== head.groupIdHex || queued.groupIdHex !== head.groupIdHex
      || next.instanceKeyHex !== queued.instanceKeyHex
      || snapshot.instanceKeyHex !== next.instanceKeyHex
      || snapshot.snapshotDigestHex !== next.snapshotDigestHex
      || next.baseRetainedSnapshotDigestHex !== retained.snapshotDigestHex
      || next.epochDec !== retained.epochDec
      || next.groupContextDigestHex !== retained.groupContextDigestHex
      || next.priorStateDigestHex !== (prior?.stateDigestHex ?? null)
      || next.sequence !== (prior?.sequence ?? 0) + 1
      || next.sentCount !== (prior?.sentCount ?? 0) + 1
      || next.receivedCount !== (prior?.receivedCount ?? 0)
      || queued.ordinal !== next.sentCount) {
      failB27(B27_ERROR.INVALID, 'outbound successor bindings are incoherent');
    }
    return this.#db.transaction(MESSAGE_TRANSACTION_STORES, async (ops) => {
      const currentHead = parseHead(await required(ops, B27_STORES.head,
        head.groupIdHex, 'canonical head'));
      if (currentHead.headDigestHex !== head.headDigestHex) {
        failB27(B27_ERROR.CAS_CONFLICT, 'canonical head changed before outbound commit');
      }
      await requireLiveRetainedState(ops, retained);
      const stateKey = messageStateKey(head.groupIdHex, next.instanceKeyHex);
      if (await ops.get(B27_STORES.messageRelease, stateKey) !== undefined) {
        failB27(B27_ERROR.RELEASED, 'message instance was released before outbound commit');
      }
      const currentRaw = await ops.get(B27_STORES.messageState, stateKey);
      const current = currentRaw === undefined ? null : parseMessageState(currentRaw);
      if ((current?.stateDigestHex ?? null) !== (prior?.stateDigestHex ?? null)) {
        failB27(B27_ERROR.CAS_CONFLICT, 'message position changed before outbound commit');
      }
      if (current === null) {
        await enforceMessageStateCapacity(ops, head.groupIdHex, next.instanceKeyHex);
      }
      for (const key of await ops.list(B27_STORES.appOutbox)) {
        if (typeof key !== 'string' || !key.startsWith(`${next.instanceKeyHex}:`)) continue;
        const existing = parseOutbox(await required(ops, B27_STORES.appOutbox, key,
          'outbox record'));
        if (existing.requestId !== queued.requestId) continue;
        if (existing.payloadDigestHex !== queued.payloadDigestHex
          || existing.recipientScope.length !== queued.recipientScope.length
          || existing.recipientScope.some((item, index) => item !== queued.recipientScope[index])) {
          failB27(B27_ERROR.REQUEST_CONFLICT, 'request id was reused with different inputs');
        }
        return Object.freeze({ status: 'duplicate', state: current, outbox: existing });
      }
      const records = [];
      for (const key of await ops.list(B27_STORES.appOutbox)) {
        if (typeof key === 'string' && key.startsWith(`${next.instanceKeyHex}:`)) {
          records.push(parseOutbox(await required(ops, B27_STORES.appOutbox, key,
            'outbox record')));
        }
      }
      const nonTerminal = records.filter((record) => ![
        B27_OUTBOX_STATE.ACKNOWLEDGED, B27_OUTBOX_STATE.FAILED_DISCARDED,
        B27_OUTBOX_STATE.INVALIDATED,
      ].includes(record.state));
      if (nonTerminal.length >= B27_LIMITS.maxOutboxPerInstance) {
        failB27(B27_ERROR.RESOURCE_LIMIT, 'non-terminal outbox cap is exhausted');
      }
      if (records.length >= B27_LIMITS.maxOutboxRecordsPerInstance) {
        failB27(B27_ERROR.RESOURCE_LIMIT, 'total outbox history cap is exhausted');
      }
      await Promise.all([
        ops.put(B27_STORES.messageSnapshot,
          messageSnapshotKey(next.instanceKeyHex, snapshot.snapshotDigestHex), snapshot),
        ops.put(B27_STORES.messageState, stateKey, next),
        ops.put(B27_STORES.appOutbox, outboxKey(next.instanceKeyHex, queued.ordinal), queued),
      ]);
      if (prior !== null && prior.snapshotDigestHex !== snapshot.snapshotDigestHex) {
        await ops.delete(B27_STORES.messageSnapshot,
          messageSnapshotKey(prior.instanceKeyHex, prior.snapshotDigestHex));
      }
      return Object.freeze({ status: 'committed', state: next, outbox: queued });
    });
  }

  async #commitInbound({ expectedHead, expectedRetained, expectedMessageState,
    nextMessageState, nextSnapshot, inbound, senderAuthority }) {
    const head = parseHead(expectedHead);
    const retained = parseRetainedState(expectedRetained);
    const prior = expectedMessageState === null ? null : parseMessageState(expectedMessageState);
    const next = parseMessageState(nextMessageState);
    const snapshot = parseMessageSnapshot(nextSnapshot);
    const delivery = parseInbound(inbound);
    const senderFields = [
      'epochDec', 'groupContextDigestHex', 'verifiedLeafDigestHex', 'senderLeafIndex',
      'senderIdentityHex', 'senderSignatureKeyHex', 'senderIdentityProofHex',
    ];
    if (delivery.disposition !== B27_INBOUND_STATE.ACCEPTED
      || next.groupIdHex !== head.groupIdHex || delivery.groupIdHex !== head.groupIdHex
      || delivery.baseRetainedSnapshotDigestHex !== retained.snapshotDigestHex
      || next.instanceKeyHex !== delivery.instanceKeyHex
      || snapshot.instanceKeyHex !== next.instanceKeyHex
      || snapshot.snapshotDigestHex !== next.snapshotDigestHex
      || next.baseRetainedSnapshotDigestHex !== retained.snapshotDigestHex
      || next.epochDec !== retained.epochDec
      || next.groupContextDigestHex !== retained.groupContextDigestHex
      || delivery.epochDec !== next.epochDec
      || delivery.groupContextDigestHex !== next.groupContextDigestHex
      || next.priorStateDigestHex !== (prior?.stateDigestHex ?? null)
      || next.sequence !== (prior?.sequence ?? 0) + 1
      || next.sentCount !== (prior?.sentCount ?? 0)
      || next.receivedCount !== (prior?.receivedCount ?? 0) + 1
      || delivery.receivedOrdinal !== next.receivedCount
      || senderAuthority === null || typeof senderAuthority !== 'object'
      || senderFields.some((field) =>
        delivery[field] !== senderAuthority[field])) {
      failB27(B27_ERROR.INVALID, 'inbound successor bindings are incoherent');
    }
    return this.#db.transaction(MESSAGE_TRANSACTION_STORES, async (ops) => {
      const currentHead = parseHead(await required(ops, B27_STORES.head,
        head.groupIdHex, 'canonical head'));
      if (currentHead.headDigestHex !== head.headDigestHex) {
        failB27(B27_ERROR.CAS_CONFLICT, 'canonical head changed before inbound commit');
      }
      await requireLiveRetainedState(ops, retained);
      const tipAuthority = await readTipAuthority(
        ops, retained.groupIdHex, retained.snapshotDigestHex, currentHead);
      if (tipAuthority.producingVerifiedLeafDigestHex !== null
        && delivery.verifiedLeafDigestHex
          !== tipAuthority.producingVerifiedLeafDigestHex) {
        failB27(B27_ERROR.CORRUPT,
          'inbound sender roster differs from its producing replay edge');
      }
      const stateKey = messageStateKey(head.groupIdHex, next.instanceKeyHex);
      if (await ops.get(B27_STORES.messageRelease, stateKey) !== undefined) {
        failB27(B27_ERROR.RELEASED, 'message instance was released before inbound commit');
      }
      const currentRaw = await ops.get(B27_STORES.messageState, stateKey);
      const current = currentRaw === undefined ? null : parseMessageState(currentRaw);
      if ((current?.stateDigestHex ?? null) !== (prior?.stateDigestHex ?? null)) {
        failB27(B27_ERROR.CAS_CONFLICT, 'message position changed before inbound commit');
      }
      const existingInbound = await ops.get(B27_STORES.appInbound,
        inboundKey(next.instanceKeyHex, delivery.ciphertextDigestHex));
      if (existingInbound !== undefined) {
        const existing = parseInbound(existingInbound);
        if (existing.baseRetainedSnapshotDigestHex
          !== delivery.baseRetainedSnapshotDigestHex) {
          failB27(B27_ERROR.CORRUPT,
            'inbound disposition replacement changes its retained base');
        }
        if (existing.disposition === B27_INBOUND_STATE.ACCEPTED) {
          return Object.freeze({ status: 'duplicate', state: prior, inbound: existing });
        }
        if (existing.disposition !== B27_INBOUND_STATE.DEFERRED) {
          failB27(B27_ERROR.STATE_CONFLICT,
            'closed inbound disposition cannot become accepted');
        }
      }
      if (current === null) {
        await enforceMessageStateCapacity(ops, head.groupIdHex, next.instanceKeyHex);
      }
      const inboundRecords = [];
      for (const key of await ops.list(B27_STORES.appInbound)) {
        if (typeof key !== 'string'
          || !key.startsWith(next.instanceKeyHex + ':')) continue;
        inboundRecords.push({ key, record: parseInbound(await required(
          ops, B27_STORES.appInbound, key, 'inbound history record')) });
      }
      let truncationWrite = null;
      let truncationDelete = null;
      if (existingInbound === undefined
        && inboundRecords.length >= B27_LIMITS.maxInboundPerInstance) {
        const candidate = inboundRecords
          .filter(({ record }) => record.disposition !== B27_INBOUND_STATE.DEFERRED)
          .sort((left, right) =>
            left.record.receivedOrdinal - right.record.receivedOrdinal
            || left.record.ciphertextDigestHex.localeCompare(
              right.record.ciphertextDigestHex))[0];
        if (candidate === undefined) {
          failB27(B27_ERROR.RESOURCE_LIMIT,
            'inbound history contains no terminal truncation candidate');
        }
        const priorRaw = await ops.get(
          B27_STORES.inboundTruncation, next.instanceKeyHex);
        const priorMarker = priorRaw === undefined
          ? null : parseInboundTruncation(priorRaw);
        truncationWrite = buildInboundTruncation({
          groupIdHex: next.groupIdHex,
          instanceKeyHex: next.instanceKeyHex,
          throughReceivedOrdinal: Math.max(
            priorMarker?.throughReceivedOrdinal ?? 0,
            candidate.record.receivedOrdinal),
          evictedCiphertextDigestHex: candidate.record.ciphertextDigestHex,
          evictedInboundDigestHex: candidate.record.inboundDigestHex,
          priorMarkerDigestHex: priorMarker?.markerDigestHex ?? null,
        });
        truncationDelete = candidate.key;
      }
      await Promise.all([
        ops.put(B27_STORES.messageSnapshot,
          messageSnapshotKey(next.instanceKeyHex, snapshot.snapshotDigestHex), snapshot),
        ops.put(B27_STORES.messageState, stateKey, next),
        ops.put(B27_STORES.appInbound,
          inboundKey(next.instanceKeyHex, delivery.ciphertextDigestHex), delivery),
        ...(truncationWrite === null ? [] : [
          ops.put(B27_STORES.inboundTruncation,
            next.instanceKeyHex, truncationWrite),
          ops.delete(B27_STORES.appInbound, truncationDelete),
        ]),
      ]);
      if (prior !== null && prior.snapshotDigestHex !== snapshot.snapshotDigestHex) {
        await ops.delete(B27_STORES.messageSnapshot,
          messageSnapshotKey(prior.instanceKeyHex, prior.snapshotDigestHex));
      }
      return Object.freeze({ status: 'accepted', state: next, inbound: delivery });
    });
  }

  async #commitDeferredInbound({ expectedHead, expectedRetained,
    tipCommitDigestHex, inbound }) {
    const head = parseHead(expectedHead);
    const retained = parseRetainedState(expectedRetained);
    const deferred = parseInbound(inbound);
    if (deferred.disposition !== B27_INBOUND_STATE.DEFERRED
      || deferred.groupIdHex !== head.groupIdHex
      || retained.groupIdHex !== head.groupIdHex
      || deferred.baseRetainedSnapshotDigestHex !== retained.snapshotDigestHex
      || deferred.instanceKeyHex !== messageInstanceKey({
        groupIdHex: retained.groupIdHex,
        tipCommitDigestHex,
        epochDec: retained.epochDec,
        groupContextDigestHex: retained.groupContextDigestHex,
        localMemberIdentityHex: head.accountKeyHex,
      })) {
      failB27(B27_ERROR.INVALID, 'deferred inbound binding is incoherent');
    }
    return this.#db.transaction(MESSAGE_TRANSACTION_STORES, async (ops) => {
      const currentHead = parseHead(await required(
        ops, B27_STORES.head, head.groupIdHex, 'canonical head'));
      if (currentHead.headDigestHex !== head.headDigestHex) {
        failB27(B27_ERROR.CAS_CONFLICT,
          'canonical head changed before deferred inbound commit');
      }
      await requireLiveRetainedState(ops, retained);
      const stateKey = messageStateKey(head.groupIdHex, deferred.instanceKeyHex);
      if (await ops.get(B27_STORES.messageRelease, stateKey) !== undefined) {
        failB27(B27_ERROR.RELEASED,
          'message instance was released before deferred inbound commit');
      }
      const key = inboundKey(deferred.instanceKeyHex, deferred.ciphertextDigestHex);
      const existingRaw = await ops.get(B27_STORES.appInbound, key);
      if (existingRaw !== undefined) {
        const existing = parseInbound(existingRaw);
        if (existing.baseRetainedSnapshotDigestHex
          !== deferred.baseRetainedSnapshotDigestHex) {
          failB27(B27_ERROR.CORRUPT,
            'duplicate deferred inbound changes its retained base');
        }
        return Object.freeze({ status: 'duplicate', inbound: existing });
      }
      const count = (await ops.list(B27_STORES.appInbound))
        .filter((item) => typeof item === 'string'
          && item.startsWith(deferred.instanceKeyHex + ':')).length;
      if (count >= B27_LIMITS.maxInboundPerInstance) {
        failB27(B27_ERROR.RESOURCE_LIMIT, 'deferred inbound history cap is exhausted');
      }
      await ops.put(B27_STORES.appInbound, key, deferred);
      return Object.freeze({ status: 'deferred', inbound: deferred });
    });
  }

  async #appendAppPublication({ expectedOutbox, nextOutbox, evidence }) {
    const prior = parseOutbox(expectedOutbox);
    const next = parseOutbox(nextOutbox);
    const record = parseAppPublication(evidence);
    if (prior.instanceKeyHex !== next.instanceKeyHex || prior.ordinal !== next.ordinal
      || record.instanceKeyHex !== prior.instanceKeyHex || record.ordinal !== prior.ordinal) {
      failB27(B27_ERROR.INVALID, 'application publication changes outbox identity');
    }
    return this.#db.transaction([B27_STORES.appOutbox, B27_STORES.appPublication],
      async (ops) => {
        const key = outboxKey(prior.instanceKeyHex, prior.ordinal);
        const current = parseOutbox(await required(ops, B27_STORES.appOutbox, key,
          'outbox record'));
        if (current.outboxDigestHex !== prior.outboxDigestHex) {
          failB27(B27_ERROR.CAS_CONFLICT, 'outbox changed before evidence append');
        }
        const prefix = `${prior.instanceKeyHex}:${String(prior.ordinal).padStart(16, '0')}:`;
        const count = (await ops.list(B27_STORES.appPublication))
          .filter((item) => typeof item === 'string' && item.startsWith(prefix)).length;
        if (record.sequence !== count + 1) {
          failB27(B27_ERROR.CAS_CONFLICT, 'application publication sequence changed');
        }
        await Promise.all([
          ops.put(B27_STORES.appPublication,
            appPublicationKey(record.instanceKeyHex, record.ordinal, record.sequence), record),
          ops.put(B27_STORES.appOutbox, key, next),
        ]);
        return Object.freeze({ outbox: next, evidence: record });
      });
  }

  async #replaceOutbox({ expectedOutbox, nextOutbox }) {
    const prior = parseOutbox(expectedOutbox);
    const next = parseOutbox(nextOutbox);
    if (prior.instanceKeyHex !== next.instanceKeyHex || prior.ordinal !== next.ordinal) {
      failB27(B27_ERROR.INVALID, 'outbox transition changes immutable identity');
    }
    return this.#db.transaction([B27_STORES.appOutbox], async (ops) => {
      const key = outboxKey(prior.instanceKeyHex, prior.ordinal);
      const current = parseOutbox(await required(
        ops, B27_STORES.appOutbox, key, 'outbox record'));
      if (current.outboxDigestHex !== prior.outboxDigestHex) {
        failB27(B27_ERROR.CAS_CONFLICT, 'outbox changed before terminal transition');
      }
      await ops.put(B27_STORES.appOutbox, key, next);
      return next;
    });
  }

  createEngineAdapter({ wasm, beforeReplay = async () => {},
    afterProbeReservation = async () => {},
    beforeOutboundCommit = async () => {},
    beforeInboundCommit = async () => {} } = {}) {
    return createBoundB27EngineAdapter({
      wasm, journal: this,
      initializeJournal: this.#initialize.bind(this),
      commitSettlement: this.#commitSettlement.bind(this),
      markUnrecoverable: this.#markUnrecoverable.bind(this),
      commitPrepared: this.#commitPrepared.bind(this),
      replaceGeneration: this.#replaceGeneration.bind(this),
      appendPublication: this.#appendPublication.bind(this),
      commitOutbound: this.#commitOutbound.bind(this),
      commitInbound: this.#commitInbound.bind(this),
      commitDeferredInbound: this.#commitDeferredInbound.bind(this),
      appendAppPublication: this.#appendAppPublication.bind(this),
      replaceOutbox: this.#replaceOutbox.bind(this),
      beforeReplay, afterProbeReservation, beforeOutboundCommit, beforeInboundCommit,
    });
  }

  close() { this.#db.close?.(); }

  destroy() { return this.#db.destroy(); }
}

export function createB27JournalForDb(db) {
  if (typeof db?.name !== 'string' || !db.name.startsWith(B27_DB_PREFIX)) {
    failB27(B27_ERROR.INVALID, 'database is outside the B2.7 namespace');
  }
  return new B27Journal(db, JOURNAL_TOKEN);
}

export async function openB27Journal({
  databaseTag,
  indexedDBImpl = globalThis.indexedDB,
  setTimeoutImpl = globalThis.setTimeout.bind(globalThis),
  clearTimeoutImpl = globalThis.clearTimeout.bind(globalThis),
} = {}) {
  assertString('databaseTag', databaseTag, { min: 1, max: 64, pattern: /^[a-z0-9-]+$/ });
  const db = await openVaultDb({
    name: `${B27_DB_PREFIX}${databaseTag}`,
    version: 1,
    migrations: { 1: migrationV1 },
    indexedDBImpl,
    setTimeoutImpl,
    clearTimeoutImpl,
  });
  return createB27JournalForDb(db);
}

export { rebuildGeneration, rebuildInput };
