...
const SOURCE_HEAD = '097fc88f05e8fc740ba67ef13ff07906d6999006';
...
const fixtureDirectory = join(styxJsDirectory, 'test', 'fixtures', 'mls-state-b2-2');
...
function assert(condition, message) {
  if (!condition) throw new Error(`B2.2 writer fixture: ${message}`);
}

if (existsSync(contextPath) || existsSync(envelopePath)) {
  throw new Error('B2.2 writer fixture outputs already exist; immutable fixture not overwritten');
}
...
moduleUrl.searchParams.set('fixture', 'b2-2-writer');
...
const groupId = encoder.encode('styx-b2-2-writer-fixture-v1');
...
const warmupAlice = encoder.encode('synthetic B2.2 writer Alice to Bob');
...
const warmupBob = encoder.encode('synthetic B2.2 writer Bob to Alice');
...
const referencePlaintext =
  'mls-state-b2-2 reference message — only a compatible restored PhaseB2 session decrypts this';
...
const replyPlaintext = 'B2.2 writer fixture restored response';
