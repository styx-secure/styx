...
// STYX_SPIKE_PROTOTYPE — exact-pin B3.2a MDK-to-Styx durability harness.
...
import { appendB32Transcript, validateB32Transcript } from '../marmot-phase-b3-2/b3-2-canonical.mjs';
import { MdkB32Peer } from '../marmot-phase-b3-2/b3-2-mdk-driver.mjs';
import {
  B32A_ERROR,
  B32A_FORMAT,
  B32A_MDK_REVISION,
...
import { B32A_MDK_SIGNER_PATH } from './b3-2a-mdk-signer.mjs';
import { StyxB32aPeer } from './b3-2a-styx-driver.mjs';
...
function parseArgs(args) {
  ...
  throw new Error('usage: b3-2a-orchestrator.mjs --run-dir PATH --private-dir PATH');
}
...
function existingPrivateChild(path) {
  ...
  if (!rel || rel === '..' || rel.startsWith(`..${sep}`)) {
    throw new Error('B3.2a private path escaped its approved root');
  }
  return real;
}
...
function validateMdkHello(value) {
  if (value?.mdk_revision !== B32A_MDK_REVISION
...
async function restartMode(privateDirectory) {
  const peer = await StyxB32aPeer.open(existingPrivateChild(privateDirectory));
...
async function runMode(args) {
  const runDirectory = prepareEmptyChild(resolve(args.runDirectory), B32A_RUN_ROOT, 'run directory');
  const privateDirectory = prepareEmptyChild(
    resolve(args.privateDirectory), B32A_PRIVATE_ROOT, 'private directory',
  );
  const transcript = [];
  let mdk;
  let report;
  ...
  try {
    ...
    const styx = await StyxB32aPeer.create(privateDirectory, mdkAccountIdentityHex);
    const keyPackage = appendB32Transcript(
      transcript, 'styx_advertise_exact_b32a_key_package', await styx.publicKeyPackage(),
    );
    mdk = new MdkB32Peer();
    ...
    appendB32Transcript(transcript, 'mdk_initialize', await mdk.request('initialize', {
      ...
      signer_script: B32A_MDK_SIGNER_PATH,
      signer_secret_path: mdkSecretPath,
    }));
    const creation = appendB32Transcript(
      transcript,
      'mdk_create_group_from_exact_b32a_key_package',
    ...
    appendB32Transcript(transcript, 'styx_welcome_durable_before_native_prepare', {
    ...
    if (joined.projection.groupIdHex !== creation.group_id_hex
      ...
      throw new B32aError(B32A_ERROR.PROJECTION_MISMATCH,
        'joined projection differs from MDK creation');
    }
    appendB32Transcript(transcript, 'styx_joined_single_cas_and_scratch_restore', {
      ...
      preparationEvidence: joined.preparationEvidence,
      projectionRecordSha256Hex: joined.head.projectionRecordSha256Hex,
      restartedProjectionRecordSha256Hex: joined.restartedProjectionRecordSha256Hex,
    ...
    report = Object.freeze({
      ...
      claim: 'B3.2a established only the exact pinned MDK-founder to exact-profile Styx-joiner embedded-tree Welcome path through durable JOINED activation and a fresh process restart.',
      testedPins: pins,
      groupIdHex: joined.projection.groupIdHex,
      memberProfiles: Object.freeze(joined.projection.members.map((member) => member.profileTag)),
      preparationClassification: joined.preparationEvidence.classification,
      retentionTimestampDifferenceAccepted:
        joined.preparationEvidence.classification === B32A_PREPARATION.RETENTION_TIMESTAMP_BOUNDED,
      welcomeSha256Hex: joined.projection.welcomeSha256Hex,
      projectionRecordSha256Hex: joined.head.projectionRecordSha256Hex,
      processRestart,
      transcriptHeadSha256,
    });
    if (!report.memberProfiles.includes(B32A_PROFILE.STYX)
      || !report.memberProfiles.includes(B32A_PROFILE.MDK)) {
      throw new B32aError(B32A_ERROR.POLICY_REJECTED, 'exact profile evidence is incomplete');
    }
  } catch (error) {
    const typed = typedFailure(error);
    if (typed && typed.code !== B32A_ERROR.PERSISTENCE_FAILED) {
      ...
      report = Object.freeze({
        ...
        disposition: 'NO-GO',
        compatibilityEstablished: false,
        exactProfileDurableRestartedWelcomeJoinEstablished: false,
        applicationTrafficTested: false,
        claim: 'B3.2a stopped at the first typed exact-pin incompatibility without weakening either peer.',
      ...
      report = Object.freeze({
        ...
        exactProfileDurableRestartedWelcomeJoinEstablished: false,
        applicationTrafficTested: false,
        claim: 'B3.2a produced no classification because its environment or durability primitive failed.',
        transcriptHeadSha256: validateB32Transcript(transcript),
      });
    }
  } finally {
