...
const ADAPTER_TOKEN = Symbol('B25C_ENGINE_ADAPTER_TOKEN');
...
function operationPayload(operationKind, operationPayloadBytes) {
  if (operationKind === 'self-update') {
    if (operationPayloadBytes.length !== 0) {
      failB25C(B25C_ERROR.INVALID, 'self-update payload must be empty');
    }
    return null;
  }
  if (operationKind === 'remove') {
    if (operationPayloadBytes.length !== 4) {
      failB25C(B25C_ERROR.INVALID, 'remove payload must be one u32 leaf index');
    }
    return new DataView(operationPayloadBytes.buffer,
      operationPayloadBytes.byteOffset, 4).getUint32(0, false);
  }
  if (operationKind !== 'add') failB25C(B25C_ERROR.INVALID, 'unsupported local operation');
...
function tipForPath(path, edgeByCommit) {
  ...
  if (edge === undefined) failB25C(B25C_ERROR.CORRUPT, 'branch tip edge is absent');
  ...
  const walk = (snapshotDigestHex, path, seenStates) => {
    ...
    for (const edge of next) {
      if (seenStates.has(edge.successorSnapshotDigestHex)) {
        failB25C(B25C_ERROR.CORRUPT, 'retained replay graph contains a cycle');
      }
...
export class B25CEngineAdapter {
  #wasm;
  #journal;
  #initializeJournal;
  #commitSettlement;
  ...
  constructor({ wasm, journal, initializeJournal, commitSettlement, markUnrecoverable, commitPrepared,
    replaceGeneration, appendPublication, beforeReplay, afterProbeReservation }, token) {
    if (token !== ADAPTER_TOKEN || typeof initializeJournal !== 'function'
      ...
      failB25C(B25C_ERROR.INVALID, 'B2.5c adapter requires journal-private capabilities');
    }
    if (!wasm?.Provider || !wasm?.PhaseB2Group || !wasm?.PhaseB2Identity
      ...
      failB25C(B25C_ERROR.INVALID, 'exact initialized Phase B2 WASM and B2.5c journal required');
    }
    if (typeof beforeReplay !== 'function' || typeof afterProbeReservation !== 'function') {
      failB25C(B25C_ERROR.INVALID, 'test scheduling hooks must be functions');
    }
    this.#wasm = wasm;
  ...
  #restore(retained, allowPending = false) {
    ...
    try {
      ...
      if (identity === undefined || group === undefined
        ...
        failB25C(B25C_ERROR.ENGINE_REJECTED, 'retained OpenMLS binding is invalid');
      }
      ...
      failB25C(B25C_ERROR.ENGINE_REJECTED, 'retained OpenMLS restore failed', {}, error);
    }
  }
  ...
  async initializeStable({ snapshotBytes, groupId, accountKey, signatureKey }) {
    ...
    try {
      ...
      if (identity === undefined || group === undefined
        ...
        failB25C(B25C_ERROR.ENGINE_REJECTED, 'initial state is not a stable identity binding');
      }
      ...
      if (error?.code) throw error;
      failB25C(B25C_ERROR.ENGINE_REJECTED, 'stable B2.5c initialization failed', {}, error);
    } finally {
      dispose({ provider, identity, group });
    }
  }
  ...
  async #tryInbound(parent, input) {
    ...
    try {
      ...
      if (!bytesEqual(before, session.provider.serialize_state())) {
        failB25C(B25C_ERROR.ENGINE_REJECTED, 'staging mutated its retained parent');
      }
      ...
      if (successor.epochDec !== projection.candidateEpochDec
        ...
        failB25C(B25C_ERROR.ENGINE_REJECTED, 'replay successor differs from projection');
      }
      const edge = buildEdge({ groupIdHex: parent.groupIdHex,
        ...
        origin: B25C_EDGE_ORIGIN.INBOUND, localGenerationDigestHex: null });
  ...
  async #tryLocal(parent, generation, pendingRetained) {
    ...
    try {
      if (!session.group.has_pending_commit(session.provider)) {
        failB25C(B25C_ERROR.CORRUPT, 'local pending snapshot lacks pending state');
      }
      projectionHandle = session.group.pending_projection(session.provider);
      if (projectionHandle === undefined) {
        failB25C(B25C_ERROR.CORRUPT, 'local pending projection is absent');
      }
      const projection = projectB2Commit(projectionHandle);
      const policyInputs = { parent: session.parent, candidate: projection,
      ...
      if (digestHex(canonicalProjectionBytes(projection)) !== generation.projectionDigestHex
        ...
        failB25C(B25C_ERROR.CORRUPT, 'local generation evidence changed during replay');
      }
  ...
  async #discover(bundle) {
    ...
    if (anchor === undefined) {
      failB25C(B25C_ERROR.UNRECOVERABLE, 'retained anchor is missing inside the horizon');
    }
    if (bundle.generations.some((item) => item.contradiction)) {
      failB25C(B25C_ERROR.UNRECOVERABLE,
    ...
    const sources = bundle.pass.closureCommitDigests.map((digest) => {
      ...
      if (input === undefined) failB25C(B25C_ERROR.CORRUPT, 'frozen input is absent');
      ...
      for (const source of sources) {
        ...
        for (const parent of candidateStates()) {
          ...
          if (source.generation !== null && pendingRetained === undefined) {
            failB25C(B25C_ERROR.UNRECOVERABLE, 'eligible local pending state is missing');
          }
        ...
        if (matches.length > 1) {
          terminal.set(source.digest, Object.freeze({ state: B25C_INPUT_STATE.AMBIGUOUS,
            reason: B25C_ERROR.AMBIGUOUS_PARENT }));
        ...
        if (matches.length === 1) {
          ...
          if (!match.allowed) {
            terminal.set(source.digest, Object.freeze({ state: B25C_INPUT_STATE.REJECTED,
              reason: match.reason }));
            discovered.delete(source.digest);
          } else {
            const prior = discovered.get(source.digest);
            if (prior !== undefined && prior.edgeDigestHex !== match.edge.edgeDigestHex) {
              failB25C(B25C_ERROR.AMBIGUOUS_PARENT,
            ...
            if (prior === undefined) {
              if (discovered.size >= B25C_LIMITS.maxEdges) {
                failB25C(B25C_ERROR.RESOURCE_LIMIT, 'retained graph edge cap is exhausted');
              }
              discovered.set(source.digest, match.edge);
              if (!stateByDigest.has(match.successor.snapshotDigestHex)) {
                if (stateByDigest.size >= B25C_LIMITS.maxStates) {
                  failB25C(B25C_ERROR.RESOURCE_LIMIT, 'retained provider-state cap is exhausted');
                }
    ...
    const updatedInputs = bundle.inputs.map((input) => {
      ...
      if (edge !== undefined) return rebuildInput(input, {
        state: B25C_INPUT_STATE.EDGE, edgeDigestHex: edge.edgeDigestHex, reason: null });
      ...
      const anchorAdvanced = BigInt(bundle.head.epochDec) - BigInt(anchor.epochDec)
        >= BigInt(B25C_LIMITS.rewindCommits);
      return rebuildInput(input, { state: anchorAdvanced ? B25C_INPUT_STATE.STALE
        : B25C_INPUT_STATE.DEFERRED, edgeDigestHex: null,
      reason: anchorAdvanced ? B25C_ERROR.STALE : 'PARENT_NOT_YET_RETAINED' });
    });
  ...
  async settle(passDigestHex) {
    ...
    try {
      ...
      if (error?.code === B25C_ERROR.UNRECOVERABLE) {
        await this.#markUnrecoverable(bundle.head);
      }
      throw error;
    }
    const branches = enumerateBranches(graph.anchor.snapshotDigestHex,
      graph.edges, B25C_LIMITS.rewindCommits + 1);
    const winner = branches[0];
    if (winner === undefined) failB25C(B25C_ERROR.CORRUPT, 'branch enumeration is empty');
    const winnerState = graph.states.find((item) =>
      item.snapshotDigestHex === winner.snapshotDigestHex);
    if (winnerState === undefined) failB25C(B25C_ERROR.CORRUPT, 'winner state is absent');
    let anchor = graph.anchor;
    let selectedPath = [...winner.path];
    const releaseCandidates = [];
    if (selectedPath.length > B25C_LIMITS.rewindCommits) {
      ...
      if (firstEdge === undefined) failB25C(B25C_ERROR.CORRUPT, 'anchor advance edge is absent');
    ...
    if (BigInt(winnerState.epochDec) < BigInt(bundle.head.epochDec)) {
      failB25C(B25C_ERROR.CORRUPT, 'settlement cannot lower the canonical epoch');
    }
    ...
    const normalizedInputs = graph.inputs.map((input) => {
      if (input.state !== B25C_INPUT_STATE.EDGE
        || retainedEdgeCommits.has(input.commitDigestHex)) return input;
      return rebuildInput(input, { state: B25C_INPUT_STATE.STALE,
        edgeDigestHex: null, reason: B25C_ERROR.STALE });
    });
    const generations = bundle.generations.map((generation) => {
      ...
      if (selectedPath.includes(generation.commitDigestHex)) {
        return rebuildGeneration(generation, { state: B25C_GENERATION_STATE.SELECTED });
      }
      if (retainedEdgeCommits.has(generation.commitDigestHex)) {
        return rebuildGeneration(generation, { state: B25C_GENERATION_STATE.LOSING });
      }
    ...
    const nextHead = buildHead({ groupIdHex: bundle.head.groupIdHex,
      ...
      seq: bundle.head.seq + 1, state: B25C_HEAD_STATE.STABLE,
      epochDec: winnerState.epochDec,
      groupContextDigestHex: winnerState.groupContextDigestHex,
    ...
    const settledPass = buildPass({ groupIdHex: bundle.pass.groupIdHex,
      ...
      state: B25C_PASS_STATE.SETTLED, rounds: graph.rounds, selectedPath });
  ...
  async prepareLocal(groupIdHex, operationKind, operationPayloadBytes = new Uint8Array()) {
    ...
    if (retained === undefined) failB25C(B25C_ERROR.UNRECOVERABLE, 'canonical state is absent');
    ...
    try {
      ...
      if (operationKind === 'self-update') {
        ...
        if (own?.leafIndex === argument) failB25C(B25C_ERROR.INVALID, 'self-removal is excluded');
      ...
      if (scope.length === 0 || new Set(scope).size !== scope.length) {
        failB25C(B25C_ERROR.INVALID, 'publication scope must contain unique peers');
      }
      ...
      const generation = buildGeneration({ groupIdHex,
        generation: generationFloor + 1, state: B25C_GENERATION_STATE.PREPARED,
  ...
  async #activeGeneration(groupIdHex) {
    ...
    if (generation === undefined) failB25C(B25C_ERROR.CORRUPT, 'active pointer target is absent');
  ...
  async recordAttempt(groupIdHex) {
    ...
    if (![B25C_GENERATION_STATE.PREPARED, B25C_GENERATION_STATE.PUBLISHING]
      .includes(generation.state)) {
      failB25C(B25C_ERROR.STATE_CONFLICT, 'attempt requires prepared generation');
    }
    const existing = view.publications.filter((item) =>
      item.artifactDigestHex === generation.commitDigestHex);
    if (existing.length >= B25C_LIMITS.maxPublicationRecordsPerGeneration - 1) {
      failB25C(B25C_ERROR.RESOURCE_LIMIT, 'publication capacity cannot reserve an outcome');
    }
    const ordinal = generation.publishAttempts + 1;
    const next = rebuildGeneration(generation, {
      state: B25C_GENERATION_STATE.PUBLISHING, publishAttempts: ordinal });
    const evidence = buildPublication({ groupIdHex: generation.groupIdHex,
      ...
      kind: B25C_PUBLICATION_KIND.ATTEMPT, attemptOrdinal: ordinal,
  ...
  recordAcknowledgement(groupIdHex, attemptOrdinal, recipientIdentityHex,
    payloadBytes = new Uint8Array()) {
    return this.#recordOutcome(groupIdHex, B25C_PUBLICATION_KIND.ACK,
      attemptOrdinal, recipientIdentityHex, payloadBytes);
  }

  recordFailure(groupIdHex, attemptOrdinal, recipientIdentityHex,
    payloadBytes = new Uint8Array()) {
    return this.#recordOutcome(groupIdHex, B25C_PUBLICATION_KIND.FAILURE,
      attemptOrdinal, recipientIdentityHex, payloadBytes);
  }

  async #recordOutcome(groupIdHex, requestedKind, attemptOrdinal, recipientIdentityHex,
    payloadBytes) {
    assertSafeInteger('attemptOrdinal', attemptOrdinal, 1, B25C_LIMITS.maxPublicationAttempts);
  ...
  async recordHistoricalAcknowledgement(groupIdHex, commitDigestHex, attemptOrdinal,
    ...
    if (generation === undefined) {
      failB25C(B25C_ERROR.UNKNOWN_GENERATION,
        'historical publication generation is unavailable', {
          commitDigestHex,
          truncationMarker: view.generationTruncation,
        });
    }
    return this.#recordGenerationOutcome(view, generation, B25C_PUBLICATION_KIND.ACK,
      attemptOrdinal, recipientIdentityHex, payloadBytes);
  }
  ...
  async #recordGenerationOutcome(view, generation, requestedKind, attemptOrdinal,
    ...
    if (!generation.recipientScope.includes(recipientIdentityHex)) {
      failB25C(B25C_ERROR.INVALID, 'publication recipient is outside exact scope');
    }
    const publications = view.publications.filter((item) =>
      item.artifactDigestHex === generation.commitDigestHex);
    const attempt = publications.find((item) => item.kind === B25C_PUBLICATION_KIND.ATTEMPT
      && item.attemptOrdinal === attemptOrdinal);
    if (attempt === undefined) failB25C(B25C_ERROR.INVALID, 'outcome lacks durable attempt');
    const terminal = TERMINAL_GENERATION_STATES.has(generation.state);
    const kind = requestedKind === B25C_PUBLICATION_KIND.ACK && terminal
      ? (generation.state === B25C_GENERATION_STATE.CANCELLED
        || generation.state === B25C_GENERATION_STATE.DISCARDED
          ? B25C_PUBLICATION_KIND.CONTRADICTION : B25C_PUBLICATION_KIND.LATE_ACK)
      : requestedKind;
    ...
    if (duplicate !== undefined) return Object.freeze({ status: 'duplicate', evidence: duplicate,
      generation });
    if (publications.length >= B25C_LIMITS.maxPublicationRecordsPerGeneration) {
      failB25C(B25C_ERROR.RESOURCE_LIMIT, 'generation evidence capacity is exhausted');
    }
    const next = rebuildGeneration(generation, {
  ...
  async cancelBeforeAttempt(groupIdHex) {
    ...
    if (generation.state !== B25C_GENERATION_STATE.PREPARED
      || generation.publishAttempts !== 0) {
      failB25C(B25C_ERROR.STATE_CONFLICT, 'cancel requires pre-attempt generation');
    }
    return this.#replaceGeneration(generation,
      rebuildGeneration(generation, { state: B25C_GENERATION_STATE.CANCELLED }),
      { clearActive: true });
  }

  async discardAfterFailure(groupIdHex) {
    const { generation } = await this.#activeGeneration(groupIdHex);
    if (generation.state !== B25C_GENERATION_STATE.PUBLISHING
      || generation.failureCount < 1 || generation.ackCount !== 0) {
      failB25C(B25C_ERROR.STATE_CONFLICT, 'discard requires failure and no ACK');
    }
    return this.#replaceGeneration(generation,
      rebuildGeneration(generation, { state: B25C_GENERATION_STATE.DISCARDED }),
      { clearActive: true });
  }

  async createLivenessProbe(groupIdHex, plaintextBytes) {
    assertBytes('probe plaintext', plaintextBytes,
      { min: 1, max: B25C_LIMITS.maxProbePayloadBytes });
  ...
  async processLivenessProbe(groupIdHex, ciphertextBytes) {
    assertBytes('probe ciphertext', ciphertextBytes,
      { min: 1, max: B25C_LIMITS.maxProbePayloadBytes * 2 });
    const bundle = await this.#journal.readHead(groupIdHex);
    if (bundle.retained === null) failB25C(B25C_ERROR.UNRECOVERABLE, 'probe state absent');
...
export function createBoundB25CEngineAdapter(options) {
  return new B25CEngineAdapter(options, ADAPTER_TOKEN);
}
