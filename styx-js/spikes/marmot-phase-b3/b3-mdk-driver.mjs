...
import { assertExactKeys, assertLowerHex } from './b3-canonical.mjs';

const scriptPath = fileURLToPath(import.meta.url);
const scriptDirectory = dirname(scriptPath);
const defaultBinary = resolve(scriptDirectory, 'mdk-peer', 'target', 'debug', 'styx-b3-mdk-peer');
const RPC_TIMEOUT_MS = 30_000;

function securePrivateFile(path) {
  const privateRoot = realpathSync('/home/mverde/.local/share/styx-b3-private/issue-165');
  const real = realpathSync(path);
  const rel = relative(privateRoot, real);
  if (!rel || rel === '..' || rel.startsWith(`..${sep}`)) {
    throw new Error('proof signer secret escaped the approved private root');
  }
...
export const B3_MDK_DRIVER_PATH = scriptPath;
export const B3_MDK_DEFAULT_BINARY = defaultBinary;
