...
| C0.3 | all seven frozen Issue #251 entry items satisfied by Issue #262; corpus construction only is authorized | no later decision may be guessed or inherited from an implementation; C0.3 remains `NO-GO` for every retained capability gate |
...
- `C0.3_CORPUS_PATH_APPROVAL` is bounded `DECIDED` by Issue #253; its six
  future paths remain absent until the separately contracted corpus task;
- Issue #262 completes the final authority-set synchronization and authorizes
  corpus construction only. C0.3 no longer blocks `corpus`, but continues to
  block implementation alignment, demo, product and sensitive use until corpus
  completion, independent implementation agreement, exit review and an
