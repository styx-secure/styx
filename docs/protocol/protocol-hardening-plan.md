...
4. **Resolve remaining C0.3 blockers.** O-07 fixes single-root genesis and
   rejects checkpoint substitution in v0; O-08 is now re-establishing the bounded
...
| O-08 | `OPEN` during ratified remediation | replacement transcript-only candidates require measurement and human selection; no product/runtime claim |
...
- O-06 and O-06c are condition-bearing `DECIDED` over the exact combined
  ...
  reopen trigger;
- O-07 is bounded `DECIDED`; O-08 is under ratified remediation and new human
  selection; O-10 remains open. O-14 is condition-bearing `DECIDED`, with
