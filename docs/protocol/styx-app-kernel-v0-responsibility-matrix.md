...
- **Status:** C0.2a normative responsibility allocation, amended by
  C0.2b/C0.2c/C0.2e/C0.2f, O-06a, O-06b-1 and O-06b-2 only for decided dependencies;
...
| `OB-K05` | Define transcript-only causal representation and classify predecessor, concurrent, duplicate, missing-parent, replay and fork relationships independently of openings and AP authority | Authenticated object set → causal classification and admitted graph | O-01/C0.2d plus C0.2i decided within bounds; C0.2j collision identity and later reruns remain mandatory |
| `OB-K06` | Derive deterministic total order only after causality; compute pending roots and their causal-descendant closure without AP input; never let replay position supply authority, priority, physical-destruction permission or an availability boundary outside that subtree | Concurrent authenticated objects plus monotone verified-opening set → unchanged graph/order, pending set and canonical applied order outside it | C0.2i v2 evidence selected; C0.2j/C0.2k/O-06c and O-08 bounds remain open |
...
| `OB-K10` | Authenticate the O-04 content descriptor; derive/verify the randomized-opening commitment; keep class, availability, binding, retention and replay readiness distinct; reject content-bearing control events; validate append-only removal structure | Payload/opening, descriptor or retained commitment/removal evidence → typed verification/readiness/pending result | O-04/C0.2f/C0.2i semantics are decided within bounds; current descriptor copy is a non-protection; C0.2k/O-06c, O-08 and O-10 remain open |
...
| `OB-AP02` | Define actor/credential types and reversible prefix-scoped role authority, delegation, rotation, revocation and expiry without redefining K binding/admission | K-authenticated historical binding plus AP prefix state → authorized, authentic-but-unauthorized or post-revocation outcome | O-02/C0.2i split decided; C0.2j identity/grant evidence and concrete profile bounds remain open |
...
| `OB-AP05` | Define retention, legal hold, append-only logical removal, export, opening retention and post-removal reconstruction by data class; authorize content-free controls without redefining K validity or pending mechanics | Authenticated `NONE` policy action plus O-04 class/evidence → reversible bounded request to K/RS/PV | O-04/C0.2i mechanics decided; profile policy, O-15 lifecycle and O-13 irreversible effects remain open |
...
| Obligation | Owned rule | Required input / emitted result | Status dependency |
| --- | --- | --- | --- |
| `OB-PV01` | Present truthful UI states for local save, queued, published, pending opening/ancestor, provisional replay, whole-projection stale, acknowledged, read, rejected and conflicted states | Typed lower-layer outcomes → user-facing state | C0.2i requires pending/provisional honesty; Themis/reference UI future work |
...
| `OB-PV11` | Communicate pending subtrees, permanent opening loss, provisional/finality limits, recovery loss, anonymity and removal limits; never promise deletion, availability, stable finality or rollback-stable removal without evidence | Profile/deployment facts → user documentation | Every product release; O-13, O-15 and O-16 gate stronger claims |
...
| O-04 payload commitment/detachment | `K` for descriptor, commitment, typed state and pending-subtree mechanics; `AP` for content-free control policy/authority; `RS` for opening custody/effects | `SS`, `TR` and `PV` supply protected delivery, authenticated fetch and truthful pending/provisional presentation | **Decided within C0.2i bounds:** descriptor-only randomized-opening commitment; authenticated `NONE`/`REQUIRED`/`DETACHABLE`; append-only logical removal; pending root plus causal descendants only; independent events proceed; no checkpoint substitution. Current 44-octet `CTX` copy non-protections remain until C0.2k. |
...
| O-07 genesis and checkpoint evidence | `K` for object authentication/binding; `AP` for any producer authorization/acceptance semantics | `RS` supplies custody/rollback bounds; `PV` supplies supportable claims | Genesis authority is the sole C0.2i static abstraction. Checkpoint substitution remains unsupported: if content- or authority-bearing replay evidence exists only in a checkpoint, the whole projection is stale. Exact genesis/checkpoint construction remains open. |
