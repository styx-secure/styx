...
| O-08 skew/cardinality/activation bounds | `AP` | `K`, `SS`, `RS`, `TR`, `PV` provide capability/resource envelopes | **`OPEN` during ratified remediation:** 46 semantic maxima, including exact authority-poset antichain width, four capability minima, one structural exact capability-key declaration and two exact-zero values are candidates; eleven post-C0.3 and four evidence-only dimensions remain non-entry. New measurement, ForgeRelay selection and exact-final gates are required. No product/runtime support follows. |
