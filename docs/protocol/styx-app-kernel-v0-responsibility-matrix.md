...
| `OB-K08` | Bind application/case context and the derived genesis reference into every authoritative object and reject cross-context replay | Context plus candidate object → accepted context or rejection | O-07 closes the genesis-authored share: one O-03 tuple/reference pair independently authenticated by a local CeremonyBoundary capability is fixed and every descendant is compared to it; Issue #260 integrates that provenance with the frozen O-14 verifier and O-06c construction; its exact-final technical gate passes and review/human gates remain |
...
| O-14 signature-suite registry | `K` for verification mechanics and registry; `AP` for admitted credential/assurance profiles | `RS` and implementation evidence supply key custody/runtime capability | **Condition-bearing `DECIDED`:** internal suite `0x0001` is pure Ed25519 over the exact O-06b-1 transcript with canonical 32/64-octet key/signature encodings, `S < L`, prime-order `A`/`R`, one pinned verifier and no alternate selector/fallback. Product adapters remain unclaimed; Issue #260 supplies the separately ratified placeholder-substituted O-06c rerun, whose exact-final technical evidence passes while review/human gates remain pending. |
...
technical gate passes, review/human gates remain, and Dart/browser product
conformance remains unclaimed. The completed
