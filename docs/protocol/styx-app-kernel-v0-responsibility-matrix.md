...
| `OB-K05` | Define transcript-only causal representation and classify predecessor, concurrent, duplicate, missing-parent, replay and fork relationships independently of openings and AP authority | Authenticated object set → causal classification and admitted graph | O-01 is decided by the ratified C0.2i evidence, which quarantines every forked v0 context; C0.2j identity/fork semantics and later reruns remain mandatory |
| `OB-K06` | Derive deterministic total order only after causality; compute pending roots and their causal-descendant closure without AP input; never let replay position supply authority, priority or physical-destruction permission; when any fork exists, retain graph/pending diagnostics but expose no AP order or producer-eligible frontier | Concurrent authenticated objects plus monotone verified-opening set → unchanged graph/order, pending set, canonical applied order or whole-context fork quarantine | Ratified bounded C0.2i evidence; C0.2j/C0.2k/O-06c and O-08 bounds remain open |
...
| `OB-K10` | Authenticate the O-04 content descriptor; derive/verify the randomized-opening commitment; keep class, availability, binding, retention and replay readiness distinct; reject content-bearing control events; validate append-only removal structure | Payload/opening, descriptor or retained commitment/removal evidence → typed verification/readiness/pending/quarantine result | O-04 is decided by ratified C0.2i evidence; C0.2i supplies only symbolic K-readable control kinds, grant subjects, binding references and verification keys that current O-06b-1 does not yet carry; current descriptor copy is a non-protection; C0.2j/C0.2k/O-06c, O-08 and O-10 remain open |
...
| --- | --- | --- | --- |
| O-01 causal representation | `K` | `AP`, `RS`, `TR` supply conflict, resource and privacy requirements | **Decided after C0.2i ratification:** per-credential chain/frontier topology remains, while any admitted same-author fork permanently quarantines the whole v0 AP context. C0.2j successor semantics remain mandatory. |
| O-02 author/rotation/authorization binding | `AP` for authority semantics; `K` for authenticated binding mechanics | `SS`, `RS`, `PV` supply member, custody and operational constraints | **Decided within the bounded C0.2i envelope:** identity/session separation remains; fork-free concurrent grant/revoke can launder successor authority in one grindable order. C0.2j must define provenance, transitivity and two-sided authority. |
