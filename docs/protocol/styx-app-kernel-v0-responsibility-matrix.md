...
- **Decision effect:** C0.2a closes O-09; C0.2b closes O-02/O-03; C0.2c closes
  ...
  and is condition-bearing `DECIDED`; O-07, O-08 and O-10 are bounded
  `DECIDED`; O-11 through O-13 remain
...
| `SS → K` | Authenticated plaintext bytes, sender/session attribution, session profile identity and a typed session outcome; observed epoch remains a model-local SS diagnostic and is not a K-interface value | `K` still validates application version, context, author binding, transcript and replay rules | Successful decryption proves application authorization, causality, freshness or business truth |
...
| `OB-K08` | Bind application/case context and the derived genesis reference into every authoritative object and reject cross-context replay | Context plus candidate object → accepted context or rejection | O-07 closes the genesis-authored share: one O-03 tuple/reference pair independently authenticated by a local CeremonyBoundary capability is fixed and every descendant is compared to it; Issue #260 / merged PR #261 integrates that provenance with the frozen O-14 verifier and O-06c construction, with exact-final review and human gates complete. |
...
| `OB-K10` | Authenticate the O-04 content descriptor; derive/verify the randomized-opening commitment; keep class, availability, binding, retention and replay readiness distinct; reject content-bearing control events; validate append-only removal structure | Payload/opening, descriptor or retained commitment/removal evidence → typed verification/readiness/pending/quarantine result | O-04 and C0.2k select unchanged-opening rejection across credential/sequence contexts plus content-free credential controls; knowledgeable recomputation remains possible. O-06c bounded evidence passes; O-08 bounds the work and bounded O-10 names the trusted-local result without selecting a wire code. |
| `OB-K11` | Distinguish empty, uninitialized and valid histories | Stored/input history → classified state | Decided by registry K-08; bounded O-10 supplies local classes only |
...
| `OB-K13` | Define logical replay and duplicate idempotency semantics for application objects | Object identifier plus history → new/duplicate/replay outcome | O-06b-1 fixes reference derivation, O-06c bounded evidence passes and O-10 fixes `DUPLICATE` as a non-applied idempotent local outcome |
...
| `OB-K16` | Emit stable, bounded protocol outcomes required for safe caller behavior without leaking parser internals | Validation site → stable classified outcome | Bounded O-10 fixes 25 trusted-local primaries and collapses every non-applied untrusted-remote result to `OPAQUE_REMOTE_FAILURE`; no numeric/wire code is selected |
...
| `OB-AP06` | Select allowed assurance capabilities and reject an internally inconsistent combination | Runtime/session/transport capability declarations → activated or rejected profile | Bounded O-08 selects four abstract C0.3 capability minima plus one exact structural four-key declaration; concrete product/runtime capability proofs remain profile work. |
...
| `OB-AP10` | Define first-profile actor/cardinality, clock-skew and activation bounds as profile rules rather than kernel limits | Capability/resource evidence → in-profile or out-of-profile result | Bounded O-08 selects candidate actor/role cardinalities, exact-zero physical-time skew and an exact four-key activation set for transcript-only C0.3. |
...
| Obligation | Owned rule | Required input / emitted result | Status dependency |
| --- | --- | --- | --- |
| `OB-SS01` | Authenticate session members and bind the exact session profile without equating membership with application role; preserve the SSD-02 prohibition on treating membership, decryption or possession as AP/K authority, including never proving causality, freshness or business truth | Exact SSD-01 profile plus abstract session/context binding → authenticated member/session attribution with no AP role result | SSD-01, SSD-02 and SSD-11 are the SS-0 Gate-A candidate; supported adapter remains future work |
| `OB-SS02` | Authenticate and stage an in-profile membership mutation before authoritative application; require the abstract RS tri-state and apply only after `COMMITTED` | In-profile candidate mutation plus RS result → rejected, staged, unapplied or authoritatively applied model result | SSD-04 Gate-A candidate; proposal-bearing/general membership remains unsupported |
| `OB-SS03` | Provide confidential authenticated session payload processing only for opaque bounded bytes under the exact evidence profile | Opaque bounded application bytes plus exact session state → bounded ciphertext/plaintext observation | SSD-01–SSD-03 Gate-A candidate; no transport, delivery or product claim |
| `OB-SS04` | Keep epoch retention, exact duplicate replay and the selected two-candidate convergence rule independent; reject out-of-profile topology before selection | Session message or exact concurrent-Commit pair plus state → bounded accepted, duplicate, stale, selected, deferred or unsupported model result | SSD-05 and SSD-06 Gate-A candidate; arbitrary convergence remains unsupported |
| `OB-SS05` | Admit only framed non-last-resort KeyPackages and the embedded-tree Welcome flow; model one-shot logical consumption without importing physical-time or rollback guarantees | Exact onboarding objects plus local logical consumption state → accepted, consumed or unsupported model result | SSD-07 Gate-A candidate; LastResort, expiry evaluation, recovery and multi-device remain open/future work |
| `OB-SS06` | State only the logical state availability needed by the five-past-epoch and retained-loser evidence; keep physical custody and deletion with RS | Bounded session transition/history state → logical available/unavailable evidence and explicit non-claim | SSD-05 and SSD-08 Gate-A candidate; shipping retention, zeroization and PCR claims remain open |
| `OB-SS07` | Represent each authoritative session mutation as one indivisible logical set, apply it only after the SSD-04 RS result and treat restored state as unvalidated pending owning-layer revalidation | Candidate logical mutation plus abstract RS tri-state → staged, unapplied, halted or applied model result | SSD-04 and SSD-09 Gate-A candidate; product persistence, restore and rollback detection remain future work |
| `OB-SS08` | Expose only closed model-local redacted observations and never promote them to stable O-10, API, wire or delivery outcomes | Bounded SS-0 failure/result → redacted model-local disposition | SSD-10 Gate-A candidate; supported adapter diagnostics remain future work |
| `OB-SS09` | Bind exact Phase-B revisions, ciphersuite, member profiles, retention depth and Gate-A source identities; invalidate rather than negotiate on drift | Exact external provenance plus candidate profile tuple → eligible or unsupported evidence result | SSD-01 and SSD-11 Gate-A candidate; no general compatibility claim |
...
| `OB-RS08` | Define storage bounds and custody for every `REQUIRED` opening in an honest frontier's causal ancestry; handle quota, eviction, suspension and background behavior without interpreting loss as removal | Runtime capability, retained frontier/openings and probe → supported/unsupported profile or typed loss | O-08 bounds transcript-only C0.3 evidence; storage/wire custody, O-11/O-13 and runtime evidence remain open. C0.2i supplies no availability guarantee. |
...
| `OB-PV04` | Maintain minimized operator/security audit access, review and incident handling without creating a universal plaintext administrator or social graph | Operational events → protected audit and response | Flegias operations future work |
...
| `OB-PV08` | Protect operators and users from abuse, spam, traumatic content and illegal material without silently defeating the anonymity profile | Abuse signal and procedure → quarantine/escalation/limit | Flegias safety design future work |
...
| O-08 skew/cardinality/activation bounds | `AP` | `K`, `SS`, `RS`, `TR`, `PV` provide capability/resource envelopes | **Bounded `DECIDED`:** the provider-bound `balanced` transcript-only entry envelope selects 46 semantic maxima, including exact authority-poset antichain width, four capability minima, one structural exact capability-key declaration and two exact-zero values; eleven post-C0.3 and five evidence-only dimensions remain non-entry. Exact `B4(P)` is proof evidence rather than a runtime gate. Two-clean-checkout evidence and exact-HEAD technical review passed. No product/runtime support follows. |
...
| O-10 stable error taxonomy | `K` for protocol outcomes; each profile for its own typed failures | All consumers supply safe-recovery distinctions | **Bounded `DECIDED`:** 25 trusted-local primaries, deterministic stage precedence, one historical alias, two forbidden post-C0.3 markers and one opaque untrusted-remote collapse. Recovery classes constrain retry but promise no success. Numeric/wire/API codes remain future profile work. |
...
| O-14 signature-suite registry | `K` for verification mechanics and registry; `AP` for admitted credential/assurance profiles | `RS` and implementation evidence supply key custody/runtime capability | **Condition-bearing `DECIDED`:** internal suite `0x0001` is pure Ed25519 over the exact O-06b-1 transcript with canonical 32/64-octet key/signature encodings, `S < L`, prime-order `A`/`R`, one pinned verifier and no alternate selector/fallback. Product adapters remain unclaimed; Issue #260 / merged PR #261 supplies the separately ratified placeholder-substituted O-06c rerun and completes its exact-final review and human gates. |
...
genesis block boundary. O-14's selected semantics are exercised by Issue #260's
separately ratified O-06c placeholder-substitution rerun; its exact-final
technical gate, review and human gates completed in merged PR #261 at
`490689f0d81980cf942d448c76a54192913b7cde`, while Dart/browser product
conformance remains unclaimed. Issue #262 authorizes only construction of the
separately contracted C0.3 corpus; it does not change any owner or obligation.
The completed
