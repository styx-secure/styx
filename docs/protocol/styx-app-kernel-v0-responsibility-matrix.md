...
| `OB-K05` | Define transcript-only causal representation and classify predecessor, concurrent, duplicate, missing-parent, replay and fork relationships independently of openings and AP authority; expose same-author fork evidence without choosing a sibling | Authenticated object set → causal classification, admitted graph and lineage-fork evidence | O-01 is decided; C0.2j replaces the bounded C0.2i whole-context response with credential-lineage containment. C0.2k/O-06c and O-08 bounds remain open |
...
| `OB-AP02` | Define actor/credential types and reversible role authority, delegation, rotation, revocation and expiry without redefining K binding/admission; compute `MustAuth` for expansion, producer eligibility and operational authority across every bounded causal linearization, and `MayAuth` for reduction; never use one replay order as terminal authority | Complete K-authenticated binding/control evidence plus the AP policy and O-07 initial authority → authorized, authentic-but-unauthorized, post-revocation, terminated-lineage or scoped-quarantine outcome | O-02/C0.2j selected in the bounded v3 model with fresh full replay; profile policy, O-08 bounds, O-10 outcomes and O-06c evidence remain open |
