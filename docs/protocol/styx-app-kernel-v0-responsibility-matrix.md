...
- **Status:** C0.2a normative responsibility allocation, amended by
  C0.2b/C0.2c/C0.2e/C0.2f/C0.2i/C0.2j, O-06a, O-06b-1 and O-06b-2 only for decided dependencies;
...
- **Decision effect:** C0.2a closes O-09; C0.2b closes O-02/O-03; C0.2c closes
  ...
  grant-rooted credential identity, two-sided set-relative authority and
...
| `OB-K05` | Define transcript-only causal representation and classify predecessor, concurrent, duplicate, missing-parent, replay and fork relationships independently of openings and AP authority; classify the complete sibling set at one `(credential_id, author_sequence)` slot as one fork without choosing a sibling | Authenticated object set → causal classification, admitted graph and lineage-fork evidence | O-01 is decided; C0.2j replaces the bounded C0.2i whole-context response with credential-lineage containment. Same-key aliases remain separate credential fork namespaces. C0.2k/O-06c and O-08 bounds remain open |
| `OB-K06` | Derive deterministic total order only after causality; compute pending roots and their causal-descendant closure without AP input; never let replay position supply authority, priority or physical-destruction permission; retain graph/pending diagnostics while the set-relative AP fold determines scoped operational authority | Concurrent authenticated objects plus monotone verified-opening set → unchanged graph/order, pending set, canonical replay inputs and scoped fork evidence | C0.2i pending/no-substitution behavior plus C0.2j set-relative authority and lineage containment selected; C0.2k/O-06c and O-08 bounds remain open |
...
| `OB-K09` | Define fresh deterministic genesis and initial authenticated authority inputs | Profile/context inputs → genesis object | K-09 and O-01/O-02/O-03/O-04/O-05 decided after C0.2i ratification; O-06/O-07 remain open |
| `OB-K10` | Authenticate the O-04 content descriptor; derive/verify the randomized-opening commitment; keep class, availability, binding, retention and replay readiness distinct; reject content-bearing control events; validate append-only removal structure | Payload/opening, descriptor or retained commitment/removal evidence → typed verification/readiness/pending/quarantine result | O-04 and the role-`0x02` content-free credential-control rule are selected; current descriptor copy is a non-protection and C0.2k/O-06c, O-08 and O-10 remain open |
...
| `OB-AP02` | Define actor/credential types and reversible role authority, delegation, rotation, revocation and expiry without redefining K binding/admission; compute per-event `MustAuth` for expansion, producer eligibility and operational authority and per-event `MayAuth` for reduction across every bounded causal linearization; separately expose possible/necessary terminal-authority sets, with operational authority equal to the necessary set; never use one replay order as terminal authority | Complete non-stale K-authenticated binding/control evidence plus the AP policy and O-07 initial authority → authorized, authentic-but-unauthorized, post-revocation, terminated-lineage or scoped-quarantine outcome; stale input → authority unavailable | O-02/C0.2j selected in the bounded v3 model with fresh full replay; rejected possible-authority grants can retain bounded descendant reduction power, and same-key aliases retain independent authority/fork scope. Profile policy, O-08 bounds, O-10 outcomes and O-06c evidence remain open |
...
| --- | --- | --- | --- |
| O-01 causal representation | `K` | `AP`, `RS`, `TR` supply conflict, resource and privacy requirements | **Decided, amended by C0.2j:** per-credential chain/frontier topology remains; a same-author fork terminates only the forked credential lineage and cannot expand authority. Independent lineages remain subject to the same set-relative AP fold. |
| O-02 author/rotation/authorization binding | `AP` for authority semantics; `K` for authenticated binding mechanics and provenance | `SS`, `RS`, `PV` supply member, custody and operational constraints | **Decided, amended by C0.2j:** non-genesis IDs are binding-grant references; only `GRANT` binds; `MustAuth` governs expansion/operation and `MayAuth` governs reduction across every bounded causal linearization; termination is provenance-transitive and succession uses fresh grants. |
