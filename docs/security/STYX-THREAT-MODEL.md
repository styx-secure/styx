...
| Application and case context | Domain separation and rejection of cross-context replay | Cross-context objects fail kernel validation | `OB-K08`, requirements from `OB-AP03` | O-07 fixes a seven-field single-root transcript, an opaque local capability issued by a preconfigured CeremonyBoundary after independent external verification, immutable accepted tuple/reference and descendant comparison. Product ceremony and durable custody remain later profile work. |
...
| Content commitment, retention and evidence | Authenticated bounded descriptor and randomized-opening binding; availability remains distinct from validity; logical removal is append-only | Preserve descriptor/commitment/removal evidence; in a fork-free, non-stale context defer each missing-opening `REQUIRED` event and its causal descendants while applying independent events; reject unchanged-opening copy across credential/sequence slots; never substitute checkpoint/relay/peer/time evidence | `OB-AP05`, mechanics by `OB-K10`/`OB-K14`, custody by `OB-RS08`, fetch by `OB-TR05`, presentation by `OB-PV01`/`OB-PV11` | Permanent opening loss can strand a subtree; selective disclosure causes temporary projection divergence; knowledgeable recomputation and same-slot forks remain possible; no finality or erasure follows. |
...
- **reconstructibility**: in v0 a fresh replica first obtains the identical
  semantic ceremony assertion through an independent out-of-band path, has its
  own local O-07 Boundary authenticate it and issue a non-transferable
  capability, accepts the matching signed genesis, then derives authoritative AP state from retained events and
  every directly verified `REQUIRED` content/opening. V0 has no compaction or
  checkpoint input; checkpoints never substitute for those inputs;
...
an opening to different replicas or amplify replay through delayed reveal.
...
that the compromised material permits and may retain any plaintext already
available.
...
events remain parent-usable graph evidence but apply no AP transition. The
C0.2j fold terminates the revoked or forked credential and every bounded grant
descendant, and rotation/recovery use separately admitted fresh grants; neither
can resurrect an old identifier. A late concurrent event still causes fresh
full replay and can reversibly change visible authority, so no irreversible
...
match and O-14 verification, fixes one root, and rejects every distinct root and
its descendants. A compromised ceremony or acceptance store remains outside
that guarantee.
...
Relays are never authoritative for genesis selection, application validity,
order, membership, opening substitution or human delivery. A relay can withhold an opening or
...
withhold required live transcript material and deny reconstructibility. It
cannot populate `checkpoint_evidence_refs`: O-07 admits no checkpoint object or
compaction in v0, so the historical suppress-side `STALE_EVIDENCE` trigger is
structurally unreachable. Checkpoint-like input is rejected before projection.
...
self-lock one control branch by withholding an ancestor opening. A self-fork
terminates that authority's credential lineage and cannot expand authority;
independent lineages continue only when definitely authorized. Conversely, one
uncontested compromised sole authority can causally remove every peer and remain
the only producer because C0.2j invents no quorum or out-of-band authority. Styx cannot guarantee handler
...
| A3 malicious peer | Plaintext, causal state, availability, erasure | Require a matching capability issued by the acceptor's own O-07 CeremonyBoundary; reject foreign/caller-fabricated handles and alternate roots; detect replay/fork/conflict; keep holes and descendants pending; terminate forked credential lineages and descendants; expose selective disclosure, scoped quarantine and delayed replay | `OB-K05`–`OB-K14`, `OB-K19`, `OB-AP04`, `OB-PV01`/`OB-PV04` | An authorized peer can copy plaintext, keep a subtree pending forever, amplify replay, terminate its lineage, exploit sole authority or lie in signed content. Root loss can make the whole context authority-unavailable. Boundary, issuer-configuration, runtime or accepted-store compromise voids the root guarantee. |
...
| A5 hostile/colluding relay | Availability, freshness, routing metadata | Verify outer objects, ignore relay order/response as genesis or application authority, reject checkpoint-like input, authenticate fetched openings, retry/fail over and measure exposure | `OB-TR01`–`OB-TR06`, `OB-TR10` | Relays can selectively withhold genesis, events or openings and collude over visible metadata; they never manufacture `R`, substitute for verification or prove freshness. |
...
| Existing commitment/opening is presented under another credential or author sequence | Recompute both leaf and outer commitment preimages with the authenticated event slot; the unchanged value must mismatch | Classify the content presentation as a binding mismatch without inferring impersonation, truth or authorization; a newly recomputed valid commitment is a distinct event claim | Application semantic kernel |
...
| Fresh replica is offered compacted history | Reject the unsupported v0 input before projection | Obtain authenticated `R`, matching genesis, retained live transcripts and directly verified REQUIRED openings; no checkpoint or compaction object exists | Application semantic kernel and application profile |
| Checkpoint material offered for AP-state substitution | Treat the capability as unsupported in v0 | Reject before projection; never trust producer eligibility, possession, horizon or AP state. Any future capability reopens O-01/O-04/O-07, O-02 when it creates producer authority, and the threat model | Application semantic kernel and application profile |
...
C0.2j's bounded v3 evidence closes the modeled identifier-rebinding and
