...
| `evidence` | bounded opaque publication evidence |
...
| PoC members | 16 |
| proposals | 32 |
| publication attempts / evidence records | 64 each |
...
| deterministic write crash or mapped quota failure | all four stores retain the complete predecessor |
...
an injected abort and observes no partial snapshot. Reproducible real quota
exhaustion was not forced: it is recorded as **manual/unexecuted**, as permitted
by the contract. The deterministic dependency-level quota failure is executable
and proves predecessor preservation.
...
The exact targeted run constructed the frozen cap of 16 MLS members. It
observed:

```text
members=16
max_snapshot_bytes=40055
wasm_before_bytes=1441792
wasm_peak_bytes=1572864
```
...
- Targeted Jest: 1 suite, 17 tests passed. Log SHA-256
  `cd462ec958a70b174982c2f1fcd1eee05a0a9e8ca2c79ce0e737807f37676159`.
- Playwright: Chromium and Firefox, 6 tests passed; each browser completed 100
  two-connection races without skip. Log SHA-256
  `6493bd7ba9e726b8975fca19857224b2554a38a271df1a383b58b7ff5c129b2c`.
...
- complete Jest: 87 suites and 1,172 tests passed; log SHA-256
  `06bc179378e6bcd796133ede5f9f92dd62e117be08d07d8bd56592a45801617b`;
...
- reference chat `npm ci && npm run build`: passed; build-log SHA-256
  `493426e77b1fea04749e3a3de1a88525ddb986f77e6811fa83b8e4405826f198`;
- agent-enforcement: 54 tests passed; log SHA-256
  `f43855b1bfa580f19bc315ec9a9e114121b5322a67a6568d1946184c6bb611cd`;
