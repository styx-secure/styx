...
function readCanonicalInput(skipCanonicalEquality = false) {
...
function schemaMatches(value, schema, root, branchTrace = null) {
  try {
    validateSchema(value, schema, root, branchTrace);
    return true;
  } catch (error) {
...
function validateSchema(value, schema, root, branchTrace = null) {
  ...
  if (Array.isArray(schema.allOf)) {
    for (const arm of schema.allOf) validateSchema(value, arm, root, branchTrace);
  }
  if (Array.isArray(schema.anyOf)) {
    requireCondition(schema.anyOf.some((arm) => schemaMatches(value, arm, root, branchTrace)), "schema anyOf has no matching arm");
  }
  if (Array.isArray(schema.oneOf)) {
    const matching = schema.oneOf
      .map((arm, index) => schemaMatches(value, arm, root, null) ? index : null)
      .filter((index) => index !== null);
    if (branchTrace !== null) {
      const topOneOf = root.oneOf;
      const requestOneOf = root.$defs?.InterfaceRequestV0?.oneOf;
      const responseOneOf = root.$defs?.InterfaceResponseV0?.oneOf;
      requireCondition(
        Array.isArray(topOneOf) && Array.isArray(requestOneOf) && Array.isArray(responseOneOf),
        "structural validation failed",
      );
      if (schema.oneOf === topOneOf) branchTrace.top.push(matching);
      else if (schema.oneOf === requestOneOf) branchTrace.request.push(matching);
      else if (schema.oneOf === responseOneOf) branchTrace.response.push(matching);
    }
    requireCondition(matching.length === 1, "schema oneOf is not exclusive");
    if (branchTrace !== null) validateSchema(value, schema.oneOf[matching[0]], root, branchTrace);
  }
  if (objectValue(schema.not)) requireCondition(!schemaMatches(value, schema.not, root, branchTrace), "schema not matched");
  if (objectValue(schema.if)) {
    const branch = schemaMatches(value, schema.if, root, branchTrace) ? schema.then : schema.else;
    if (objectValue(branch)) validateSchema(value, branch, root, branchTrace);
  }
  if (typeof value === "string") {
  ...
  if (Array.isArray(value)) {
    ...
    if (objectValue(schema.items)) for (const item of value) validateSchema(item, schema.items, root, branchTrace);
  }
  if (objectValue(value)) {
    ...
    if (objectValue(schema.properties)) {
      for (const [name, child] of Object.entries(schema.properties)) {
        if (Object.prototype.hasOwnProperty.call(value, name)) validateSchema(value[name], child, root, branchTrace);
      }
...
function validateCompleteV2Document(document, schema, trustedDirection) {
  requireCondition(["REQUEST", "RESPONSE"].includes(trustedDirection), "structural validation failed");
  requireCondition(objectValue(document), "structural validation failed");
  const trace = { top: [], request: [], response: [] };
  validateSchema(document, schema, schema, trace);
  const topLevelArmIndex = trustedDirection === "REQUEST" ? 0 : 1;
  requireCondition(typeof document.operation === "string", "structural validation failed");
  const operationIndex = SUPPORTED_OPERATIONS.indexOf(document.operation);
  requireCondition(operationIndex >= 0, "structural validation failed");
  const nested = trustedDirection.toLowerCase();
  const other = nested === "request" ? "response" : "request";
  requireCondition(deepEqual(trace.top, [[topLevelArmIndex]]), "structural validation failed");
  requireCondition(deepEqual(trace[nested], [[operationIndex]]), "structural validation failed");
  requireCondition(trace[other].every((matches) => matches.length === 0), "structural validation failed");
  return Object.freeze({ topLevelArmIndex, nestedOperationArmIndex: operationIndex });
}
...
function parseArguments(argv) {
  ...
  if (validateV2EvidenceMode) {
    requireCondition(["REQUEST", "RESPONSE"].includes(trustedDirection), "--direction is required for V2 evidence");
  } else {
    requireCondition(trustedDirection === null && schemaOverride === null && !v1DetectorMutant, "V2-only argument used in another mode");
  }
  return {
...
try {
  ...
  } else if (validateV2EvidenceMode) {
    const manifestSchema = readManifestBoundJson(authority, "APP-CORE-IFACE-0-SCHEMA-CANDIDATE.json");
