...
def _request_validator(authority: ContractAuthority) -> Draft202012Validator:
    return Draft202012Validator(
        {
            "$schema": authority.schema["$schema"],
            "$ref": "#/$defs/InterfaceRequestV0",
    ...
    if isinstance(candidate, dict):
        ...
        if isinstance(transcript, str) and isinstance(
            ...
            if len(transcript) > maximum_hex_characters:
                raise RequestRejected()
    if not _request_validator(authority).is_valid(request):
        raise RequestRejected()
